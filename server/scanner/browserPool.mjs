...
export function ensurePlaywrightBrowser() {
  ...
  console.warn('[scanner] Playwright Chromium is missing. Installing it now...')
  ...
  if (result.status !== 0) {
    throw new Error('Playwright Chromium could not be installed automatically. Run "npx playwright install chromium" manually.')
  }

  if (!fs.existsSync(chromium.executablePath())) {
    throw new Error(`Playwright Chromium still not found at ${chromium.executablePath()}`)
  }
}
