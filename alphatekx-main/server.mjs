...
import * as telegramProvider from './server/telegramProvider.mjs'
...
const server = http.createServer(async (req, res) => {
  ...
  // ─── Alpha Connector (Composio) Routes ───────────────────────────────
  if (req.url === '/api/connected-apps' && req.method === 'GET') {
    try {
      ...
      return json(res, 500, { error: error instanceof Error ? error.message : 'Failed to list connected apps' })
    }
  }
  if (req.url?.startsWith('/api/connect/') && req.method === 'POST') {
    const match = req.url.match(/^\/api\/connect\/([^\/]+)\/?$/)
    if (match) {
      try {
        const config = supabaseConfig()
        const user = await currentOrLocalUser(req, config.url, config.anon)
        if (!user) return json(res, 401, { error: 'Authentication required' })
        const body = await readBody(req)
        const callbackUrl = body.callbackUrl || String(getRequestOrigin(req)) + '/api/connect/callback'
        const result = await alphaConnector.startConnection(user, match[1], callbackUrl)
        return json(res, 200, result)
      } catch (error) {
  ...
  if (req.url?.startsWith('/api/connect/') && req.method === 'GET') {
    const match = req.url.match(/^\/api\/connect\/([^\/]+)\/status\/?$/)
    if (match) {
      try {
        const config = supabaseConfig()
        const user = await currentOrLocalUser(req, config.url, config.anon)
        if (!user) return json(res, 401, { error: 'Authentication required' })
        const result = await alphaConnector.getConnectionStatus(user, match[1])
        return json(res, 200, result)
      } catch (error) {
        return json(res, 500, { error: error instanceof Error ? error.message : 'Status check failed' })
      }
    }
  }
  if (req.url === '/api/connect/callback' && req.method === 'GET') {
    const url = new URL(req.url || '/', publicAppUrl())
    const provider = url.searchParams.get('provider') || ''
    const status = url.searchParams.get('status') || 'connected'
    const error = url.searchParams.get('error') || ''
    const redirectUrl = new URL('/connectors', publicAppUrl())
    redirectUrl.searchParams.set('connected', provider || error ? 'error' : 'success')
  ...
  if (req.url?.startsWith('/api/reconnect/') && req.method === 'POST') {
    const match = req.url.match(/^\/api\/reconnect\/([^\/]+)\/?$/)
    if (match) {
      try {
        const config = supabaseConfig()
        const user = await currentOrLocalUser(req, config.url, config.anon)
        ...
        const body = await readBody(req)
        const callbackUrl = body.callbackUrl || String(getRequestOrigin(req)) + '/api/connect/callback'
        const result = await alphaConnector.reconnectProvider(user, match[1], callbackUrl)
        return json(res, 200, result)
      } catch (error) {
        return json(res, 502, { error: error instanceof Error ? error.message : 'Reconnect failed' })
      }
    }
  }
  if (req.url?.startsWith('/api/disconnect/') && req.method === 'DELETE') {
    const match = req.url.match(/^\/api\/disconnect\/([^\/]+)\/?$/)
    if (match) {
      try {
        const config = supabaseConfig()
        const user = await currentOrLocalUser(req, config.url, config.anon)
        if (!user) return json(res, 401, { error: 'Authentication required' })
        const result = await alphaConnector.disconnectProvider(user, match[1])
        return json(res, 200, result)
      } catch (error) {
        return json(res, 502, { error: error instanceof Error ? error.message : 'Disconnect failed' })
      }
    }
  }
  if (req.url?.startsWith('/api/execute/') && req.method === 'POST') {
    const match = req.url.match(/^\/api\/execute\/([^\/]+)\/([^\/]+)\/?$/)
    if (match) {
      try {
        const config = supabaseConfig()
        const user = await currentOrLocalUser(req, config.url, config.anon)
        if (!user) return json(res, 401, { error: 'Authentication required' })
        const body = await readBody(req)
        const result = await alphaConnector.executeProviderAction(user, match[1], match[2], body.params || {})
        return json(res, 200, result)
      } catch (error) {
        return json(res, 502, { error: error instanceof Error ? error.message : 'Execution failed' })
      }
    }
  }
  if (req.url?.startsWith('/api/connected-apps/executions/') && req.method === 'GET') {
    const match = req.url.match(/^\/api\/connected-apps\/executions\/([^\/]+)\/?$/)
    if (match) {
      try {
        const config = supabaseConfig()
        const user = await currentOrLocalUser(req, config.url, config.anon)
        if (!user) return json(res, 401, { error: 'Authentication required' })
        const result = await alphaConnector.getExecutionHistory(user, match[1])
        return json(res, 200, result)
      } catch (error) {
