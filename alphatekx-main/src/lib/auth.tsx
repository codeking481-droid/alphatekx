      ...
      const profilePromise = (async () => {
        ...
        if (!data) {
          try {
            ...
            // RPC may not exist - fall back to insert
            const { error: insertError } = await supabase.from('profiles').insert({ id: auth.user.id, email: auth.user.email, credits: 30, plan: 'free' }).maybeSingle()
            if (insertError) {
              // Profile may have been inserted by another process
        ...
        if (data) {
          try {
            ...
            // Credit hydration failure should not block profile loading
          }
          setProfile(data as Profile)
        } else {
          // Still no profile after all attempts - set minimal profile
          setProfile({ id: auth.user.id, email: auth.user.email || '', credits: 30, plan: 'free', revenue: 0 })
        }
      })()
      await Promise.race([
        profilePromise,
        new Promise((_, reject) => setTimeout(() => reject(new Error('Profile load timed out')), 15000))
      ])
    } catch (error) {
      console.error('[Auth] refreshProfile failed:', error)
      // Ensure loading resolves even on error - set empty profile to unblock UI
      setProfile(null)
    }
  }
