...
assert.match(server, /confirmedPlatformCount > 0 && failedCount > 0 \? 'partial'/, 'mixed provider results must be reported as partial success instead of total failure')
assert.match(server, /Alpha will retry only the missing platforms/, 'partial publication must explain that confirmed platforms will not be duplicated')
assert.match(active, /execution\.status === 'partial'/, 'the UI must show partial provider confirmation instead of replacing it with a generic failure')
assert.match(server, /const publishablePosts = \(campaign\.posts \|\| \[\]\)[\s\S]*post\.approved === true/, 'only explicitly approved posts may be published')
assert.match(server, /const duePosts = publishablePosts\.filter\(post => new Date\(post\.scheduledAt\)\.getTime\(\) <= now\.getTime\(\)\)/, 'scheduled execution must only select posts whose due time has arrived')
assert.match(server, /trigger === 'manual' && duePosts\.length === 0 && publishablePosts\.length > 0\) duePosts\.push\(publishablePosts\[0\]\)/, 'an explicit manual run may select only the earliest approved unpublished post')
