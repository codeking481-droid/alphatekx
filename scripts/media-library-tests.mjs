...
const tests = [
  ...
  ['premium image generation remains available in the Media Library', page.includes('Create with Alpha') && page.includes('createSmartImage')],
  ['verified video generation persists into the private Media Library', service.includes('export async function generateVideo') && service.includes('auto-generated-video') && server.includes('/api/media/generate-video') && client.includes('createSmartVideo')],
  ['generated videos retry and require verified video bytes', service.includes('downloadVideo') && service.includes('attempt < 3') && service.includes("mime.startsWith('video/')")],
  ['compact media cards support secure preview and download', page.includes('xl:grid-cols-5') && page.includes('downloadItem') && page.includes('<Download')],
  ...
  ['Publish action uses the same confirmed provider execution path', service.includes('export async function publishMediaNow') && page.includes('publishNow(item)') && server.includes('/publish$/i')],
