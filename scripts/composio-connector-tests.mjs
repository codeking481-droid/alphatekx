...
async function runTests() {
  ...
  assert(serviceSource.includes('normalizedEmail || `${composioAppName}:${alphaUserId}`'), 'Composio resolves existing email entities with a deterministic fallback')
  assert(serviceSource.includes('COMPOSIO_TOOLKIT_VERSION_'), 'Tool execution supports pinned toolkit versions')
  ...
  assert(serviceSource.includes('confirmedPublishedContentId(pid, responseData)'), 'Execution requires a real provider ID')
  ...
  assert(serviceSource.includes('if (!accountUser) return true'), 'Disconnect supports Composio responses where deprecated user_id is omitted')
  assert(serviceSource.includes('for (const account of items)') && serviceSource.includes('deletedAccounts += 1'), 'Disconnect removes every user-scoped account for the provider')
