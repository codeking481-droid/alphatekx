...
await test('UI: clean chat and product empty states exist',()=>{assert(source('src/pages/Home.tsx').includes('Ask Alpha anything')&&source('src/pages/Agents.tsx').includes('What can Alpha do for you?')&&source('src/pages/History.tsx').includes('No automations yet'),'chat or empty states missing')})
await test('UI: mobile responsive contracts exist',()=>{const home=source('src/pages/Home.tsx');const automations=source('src/pages/Agents.tsx');const connectors=source('src/pages/Connectors.tsx');assert(home.includes('max-w-[820px]')&&automations.includes('lg:grid-cols')&&connectors.includes('sm:grid-cols-2')&&connectors.includes('lg:grid-cols-3'),'responsive contracts missing')})
