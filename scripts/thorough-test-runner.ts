...
await test('Credits: low-credit warning and account route exist',()=>{const ui=source('src/components/workspace/WorkspaceLayout.tsx');assert(ui.includes('credits < 5')&&ui.includes("'/account'"),'low-credit account path missing')})
...
await test('Gmail: OAuth state is signed and tokens are encrypted',()=>{const server=source('server.mjs');assert(server.includes("createHmac('sha256'")&&server.includes('timingSafeEqual')&&server.includes('encryptSecret(tokens.access_token')&&server.includes('encryptSecret(refreshToken'),'secure Gmail OAuth contract missing')})
...
await test('UI: clean chat and product empty states exist',()=>{assert(source('src/pages/Home.tsx').includes('Ask Alpha anything')&&source('src/pages/Creations.tsx').includes('No creations yet')&&source('src/pages/Marketplace.tsx').includes('Nothing here yet'),'chat or empty states missing')})
await test('UI: mobile responsive contracts exist',()=>{const home=source('src/pages/Home.tsx');const missionUi=source('src/pages/Builder.tsx');const market=source('src/pages/Marketplace.tsx');assert(home.includes('max-w-[820px]')&&missionUi.includes('lg:grid-cols')&&market.includes('md:grid-cols'),'responsive contracts missing')})
