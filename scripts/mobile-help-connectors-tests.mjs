...
const [app, help, layout, connectors, connectorApi, home, media, mediaService, css] = await Promise.all([
  read('src/App.tsx'),
  ...
  read('src/pages/MediaLibrary.tsx'),
  read('server/mediaLibraryService.mjs'),
  read('src/index.css'),
])
...
const checks = [
  ...
  ['Media Library explains setup without leaking a schema-cache error', media.includes('Media Library is being prepared') && !media.includes('schema cache')],
  ['missing media schema does not spam the minute scheduler', mediaService.includes('if (isMissingMediaSchema(error)) return []')],
]
