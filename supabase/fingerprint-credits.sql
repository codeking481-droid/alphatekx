...
CREATE OR REPLACE FUNCTION public.grant_google_signup_credit(
  p_user_id UUID,
  p_google_sub TEXT,
  p_email TEXT
)
...
BEGIN
  IF p_user_id IS NULL OR length(trim(p_google_sub)) < 2 THEN
    RAISE EXCEPTION 'Verified Google identity required';
  END IF;

  PERFORM pg_advisory_xact_lock(hashtextextended(p_google_sub, 2));
  INSERT INTO public.profiles(id, email, credits, plan, monthly_credits, purchased_credits)
...
REVOKE ALL ON FUNCTION public.grant_google_signup_credit(UUID, TEXT, TEXT) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.grant_google_signup_credit(UUID, TEXT, TEXT) TO service_role;
...
BEGIN
  ...
  PERFORM public.grant_google_signup_credit(p_user_id, p_google_sub, p_email);
  ...
  RETURN QUERY SELECT true, 'bonus_unlocked', current_credits + bonus_delta, bonus_delta;
END;
$$;
