...
const server = http.createServer(async (req, res) => {
  ...
  if (req.method === 'POST' && req.url === '/api/alpha/repair') {
    ...
    try {
      ...
      const llmTextCall = async (messages) => {
        const result = await callLLMForRole('content', messages[0]?.content || '', messages.slice(1).map(m => m.content).join('\n\n'), { jsonMode: false, maxTokens: 1500 })
      ...
      if (isRepairRequest(message)) {
        await runRepairPipeline(message, sendEvent, llmCall)
      } else {
        await runConversationChat(message, history, sendEvent, llmTextCall)
      }
    } catch (err) {
      console.error('[REPAIR] Pipeline error:', err)
      if (!res.writableEnded) {
        res.write(`data: ${JSON.stringify({ type: 'error', message: err.message || 'Repair pipeline failed' })}\n\n`)
      }
    } finally {
  ...
  if (req.method === 'GET' && req.url?.startsWith('/api/auth/github/callback')) {
    return handleGitHubCallback(req, res)
  }
  if (req.method === 'GET' && req.url === '/api/github/status') {
    return handleGitHubStatus(req, res)
  }
  if (req.method === 'GET' && req.url === '/api/github/repos') {
    return handleGitHubRepos(req, res)
  }
