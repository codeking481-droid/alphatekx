...
export type Agent = {
  ...
  campaign?: {
    ...
    meta: { platforms: string[]; slots: { label: string; hour: number; minute: number }[]; durationDays: number; postsPerDay: number; totalPosts: number; startDate: string; endDate?: string; includeImages: boolean; imageRequested?: boolean; timezone: string; frequency?: string; frequencyText: string; publishingMode?: 'once_now' | 'once_later' | 'recurring'; durationSource?: 'user_explicit' | 'user_confirmed' | 'unresolved'; scheduleSource?: 'user_explicit' | 'user_confirmed' | 'unresolved'; daysOfWeek?: string[]; postingOption?: 'now' | 'later' | 'recurring'; localDate?: string | null; localTime?: string | null }
    posts: { id: string; day: number; slot: string; scheduledAt: string; platforms: string[]; topic: string; postType: string; captions: Record<string, string>; status: string; result: Record<string, unknown>; baseCredits?: number; credits: number; imageUrl?: string; imageStoragePath?: string; imagePrompt?: string; imageKeywords?: string[]; imageSource?: string; approved?: boolean; charged?: boolean; chargedAt?: string; edited?: boolean; reviewedAt?: string; postedAt?: string; providerPostId?: string; providerPostIds?: Record<string, string>; providerUrl?: string; executionKey?: string; publishStartedAt?: string; retryCount?: number; lastError?: string; chargeStatus?: string; timezone?: string; postingOption?: string; scheduledLocalDate?: string | null; scheduledLocalTime?: string | null }[]
    totalCredits: number
    status: string
    charged: boolean
    approved: boolean
