...
const statusBadge = (status: string) => {
  ...
  if (status === 'coming-soon') return { label: 'Coming Soon', className: 'bg-violet-500/10 text-white/55' }
  return { label: 'Available', className: 'bg-violet-500/10 text-white/55' }
}
    ...
    <div className="mx-auto w-full max-w-2xl">
      ...
      <div ref={wrapperRef} className={cn('relative w-full', title && 'mt-12')}>
        <button
          ...
          aria-expanded={open}
          className={cn(
            'flex w-full items-center justify-between rounded-2xl border border-violet-400/20 bg-violet-500/10 px-5 py-4 text-left transition hover:bg-violet-500/10',
            open && 'ring-1 ring-violet-500/50'
          )}
        >
          <div className="flex min-w-0 items-center gap-3">
            <span className={cn('grid h-10 w-10 place-items-center rounded-xl', selectedApps.length ? 'bg-gradient-to-br from-violet-900/40 to-fuchsia-900/40 text-violet-300' : 'bg-violet-500/10 text-white/55')}>
              {triggerIcon}
            </span>
          ...
          <div className="flex items-center gap-2">
            {selectedApps.length > 0 && (
              <button
                ...
                className="rounded-lg p-1 text-white/50 hover:bg-violet-500/10 hover:text-white"
        ...
        {open && (
          <div
            role="listbox"
            className="absolute left-0 right-0 top-full z-20 mt-2 flex max-h-96 flex-col rounded-2xl border border-violet-400/20 bg-[#0B0215]/95 p-2 shadow-2xl backdrop-blur-2xl"
          >
            <div className="relative p-2 pb-1">
              ...
              <input
                ...
                className="w-full rounded-xl border border-violet-400/20 bg-violet-500/10 py-2 pl-9 pr-3 text-sm text-white placeholder:text-white/40 outline-none focus:border-violet-500/50"
              ...
              {filtered.map((app) => {
                ...
                return (
                  <div
                    ...
                    className={cn(
                      'flex cursor-pointer items-center justify-between rounded-xl border-b border-violet-400/20 px-3 py-3 transition last:border-0 hover:bg-violet-500/10',
                    ...
                    <div className="flex items-center gap-3">
                      <span
                        className={cn(
                          ...
                          isSelected ? 'border-violet-500 bg-violet-500/20 text-violet-300' : 'border-violet-400/20 text-transparent'
                        )}
                      >
                        <Check size={12} strokeWidth={3} />
                      </span>
                      <span className="grid h-9 w-9 place-items-center rounded-lg bg-violet-500/10 text-white/55">
                        {app.icon}
                      </span>
            ...
            {allowSetDefault && (
              <div className="border-t border-violet-400/20 p-2">
                <div className="flex items-center justify-between gap-2">
                  ...
                  <button
                    onClick={clearAll}
                    className="rounded-xl border border-violet-400/20 px-3 py-2 text-sm text-white/70 transition hover:bg-violet-500/10"
                  >
                    Clear
                  </button>
                </div>
