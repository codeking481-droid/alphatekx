    ...
    <section className="rounded-2xl border border-violet-400/20 bg-violet-500/10 p-5">
      ...
      {!preview ? (
        <div
          ...
          className={`mt-4 grid cursor-pointer place-items-center rounded-2xl border-2 border-dashed p-8 text-center transition ${drag ? 'border-indigo-500 bg-indigo-500/10' : 'border-violet-400/20 bg-[#0A0F1E]/45 hover:border-violet-400/20'}`}
        >
          ...
          <button onClick={e => { e.stopPropagation(); openCamera() }} className="mt-4 flex items-center gap-2 rounded-lg border border-violet-400/20 px-4 py-2 text-xs transition hover:bg-violet-500/10"><Camera size={14}/> Use camera</button>
        </div>
      ) : (
        <div className="mt-4 space-y-3">
          <div className="relative overflow-hidden rounded-2xl border border-violet-400/20 bg-[#0A0F1E]/45">
          ...
          <input value={question} onChange={e => setQuestion(e.target.value)} onKeyDown={e => e.key === 'Enter' && !busy && analyze()} placeholder="Ask about this image (e.g. 'Find bugs', 'Read text', 'Describe UI')" className="w-full rounded-xl border border-violet-400/20 bg-[#0A0F1E]/45 px-4 py-2 text-sm outline-none focus:border-indigo-500" />
