...
function Header() {
  ...
  return <header className="fixed inset-x-0 top-0 z-50 border-b border-[#E4E7EC] bg-white/[.04] backdrop-blur-xl">
    <div className="mx-auto flex h-[72px] max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
      ...
      <button onClick={() => setOpen(value => !value)} className="grid size-11 place-items-center rounded-xl border border-[#E4E7EC] text-white md:hidden" aria-label="Toggle navigation">{open ? <X size={20}/> : <Menu size={20}/>}</button>
    </div>
    <AnimatePresence>{open && <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="overflow-hidden border-t border-[#E4E7EC] bg-white/[.04] md:hidden"><nav className="grid gap-1 px-4 py-4">{nav.map(([label, href]) => <a key={href} href={href} onClick={() => setOpen(false)} className="min-h-11 rounded-xl px-4 py-3 font-semibold text-slate-200 hover:bg-violet-500/10">{label}</a>)}<Link to="/auth" className="mt-2 min-h-11 rounded-xl bg-[#6941C6] px-4 py-3 text-center font-semibold text-white">Start automating</Link></nav></motion.div>}</AnimatePresence>
  </header>
}
...
function FloatingHeroSignals() {
  ...
  return <>{cards.map(({ className, icon: Icon, text, meta, delay }) => <motion.div key={text} animate={reduce ? undefined : { y: [0, -4, 0] }} transition={{ duration: 4.2, repeat: Infinity, ease: 'easeInOut', delay }} className={`absolute z-[2] items-center gap-3 rounded-2xl border border-white/70 bg-white/[.04] px-4 py-3 shadow-[0_16px_45px_rgba(16,24,40,.10)] backdrop-blur-xl ${className}`}><span className="grid size-9 place-items-center rounded-xl bg-white/[.04] text-violet-300"><Icon size={17}/></span><span><strong className="block text-xs font-black text-white">{text}</strong><small className="mt-0.5 block text-[10px] font-bold text-[#039855]">{meta}</small></span></motion.div>)}</>
}
...
function FutureOrbit() {
  ...
  return <div aria-hidden className="future-orbit pointer-events-none absolute -right-28 -top-32 size-[430px] opacity-70 sm:-right-16">
    ...
    <div className="absolute inset-[37%] rounded-full bg-white/[.04] shadow-[0_18px_70px_rgba(105,65,198,.18)]"/>
  </div>
}
...
function CommandMockup() {
  ...
  return <motion.div initial={{ opacity: 0, y: 26 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: .25, type: 'spring', stiffness: 80 }} className="relative mx-auto w-full max-w-[590px]">
    <div className="rounded-[24px] border border-white/[.10] bg-white/[.04] p-3 shadow-[0_32px_80px_rgba(16,24,40,.16)] sm:p-4">
      <div className="overflow-hidden rounded-[18px] border border-white/[.10] bg-[#0A0F1E]">
        ...
        <div className="flex items-center justify-between border-b border-white/[.10] bg-white/[.04] px-4 py-4 sm:px-5"><div className="flex items-center gap-3"><span className="grid size-9 place-items-center rounded-xl bg-[#6941C6] text-white"><Bot size={18}/></span><div><p className="text-xs font-bold text-violet-300">COMMAND CENTRE</p><p className="text-sm font-black text-white">Plan with Alpha</p></div></div><span className="flex items-center gap-2 rounded-lg border border-white/[.10] bg-[#0A0F1E] px-2.5 py-1.5 text-[10px] font-black text-slate-400"><kbd>⌘</kbd><kbd>K</kbd></span></div>
        <div className="p-4 sm:p-6">
          <div className="rounded-2xl border border-white/[.10] bg-white/[.04] p-5 shadow-[0_1px_2px_rgba(16,24,40,.05)]">
            ...
            <div className="mt-4 flex flex-wrap gap-2">{question === 0 ? ['LinkedIn','X','Instagram'].map(item => <span key={item} className="rounded-xl border border-[#D6BBFB] bg-violet-500/10 px-3 py-2 text-xs font-bold text-violet-300">{item}</span>) : question === 1 ? ['Mon–Fri','Every day','Custom'].map(item => <span key={item} className="rounded-xl border border-white/[.10] bg-white/[.04] px-3 py-2 text-xs font-bold text-slate-200">{item}</span>) : ['Yes, 9am','Choose different'].map(item => <span key={item} className="rounded-xl border border-white/[.10] bg-white/[.04] px-3 py-2 text-xs font-bold text-slate-200">{item}</span>)}</div>
          </div>
          <div className="mt-4 rounded-2xl border border-white/[.10] bg-white/[.04] px-5 py-3"><BrainWave/></div>
        </div>
      </div>
    </div>
    <motion.div animate={reduce ? undefined : { y: [0, -8, 0] }} transition={{ duration: 3, repeat: Infinity }} className="absolute -bottom-8 -left-2 rounded-2xl border border-white/[.10] bg-white/[.04] p-4 shadow-[0_16px_40px_rgba(16,24,40,.14)] sm:-left-12"><p className="text-[10px] font-bold uppercase tracking-[.12em] text-slate-400">Credits</p><div className="mt-1 flex items-center gap-2"><span className="text-xl font-black text-white">30</span><span className="rounded-full bg-[#ECFDF3] px-2 py-1 text-[10px] font-bold text-[#027A48]">Not charged yet</span></div></motion.div>
    <motion.div animate={reduce ? undefined : { y: [0, 8, 0] }} transition={{ duration: 3.5, repeat: Infinity }} className="absolute -right-2 -top-8 hidden rounded-2xl border border-white/[.10] bg-white/[.04] p-4 shadow-[0_16px_40px_rgba(16,24,40,.14)] sm:block"><p className="flex items-center gap-2 text-xs font-black text-[#027A48]"><i className="size-2 rounded-full bg-[#12B76A]"/>Automation live</p><p className="mt-1 text-xs font-semibold text-slate-400">12/30 completed</p><div className="mt-2 h-1.5 w-32 overflow-hidden rounded-full bg-[#EAECF0]"><motion.i initial={{ width: 0 }} animate={{ width: '40%' }} transition={{ delay: .8, duration: 1 }} className="block h-full rounded-full bg-[#6941C6]"/></div></motion.div>
  </motion.div>
}
  ...
  return <section id="hero" className="relative overflow-hidden bg-[#0A0F1E] px-4 pb-28 pt-32 sm:px-6 lg:pb-36 lg:pt-40">
    ...
    <div className="relative mx-auto grid max-w-7xl items-center gap-16 lg:grid-cols-[1.02fr_.98fr]">
      <motion.div initial="hidden" animate="visible" variants={{ visible: { transition: { staggerChildren: .09 } } }} className="text-center lg:text-left">
        <motion.div variants={fadeUp} className="inline-flex flex-wrap items-center justify-center gap-3 rounded-full border border-white/[.10] bg-white/[.04] px-3 py-2 shadow-[0_1px_2px_rgba(16,24,40,.05)] lg:justify-start"><AvatarStack/><span className="text-xs font-bold text-slate-200">Trusted by 2,000+ builders</span><span className="hidden h-4 w-px bg-[#D0D5DD] sm:block"/><span className="flex items-center gap-1 text-xs font-black text-slate-200"><Star size={13} fill="#FDB022" stroke="#FDB022"/>4.9</span></motion.div>
...
function ProblemSolution() {
  ...
  return <section className="bg-[#0A0F1E] px-4 py-24 sm:px-6 lg:py-32"><div className="mx-auto max-w-7xl"><SectionHeading eyebrow="From busywork to leverage" title="Your time is too valuable for repeat work." copy="The future is not another complicated dashboard. It is a trusted employee that understands the outcome."/><div className="mt-14 grid gap-5 lg:grid-cols-3">{cards.map(([title, problem, solution], index) => <motion.article whileHover={{ y: -6 }} key={title} className="rounded-2xl border border-white/[.10] bg-white/[.04] p-7 shadow-[0_1px_2px_rgba(16,24,40,.05)]"><span className="grid size-11 place-items-center rounded-xl bg-violet-500/10 text-lg font-black text-violet-300">0{index + 1}</span><h3 className="mt-6 text-xl font-black text-white">{title}</h3><p className="mt-3 leading-7 text-slate-400">{problem}</p><div className="my-5 h-px bg-[#EAECF0]"/><p className="flex gap-2 font-semibold leading-7 text-slate-200"><CheckCircle2 className="mt-1 shrink-0 text-[#12B76A]" size={18}/>{solution}</p></motion.article>)}</div></div></section>
}
...
function HowItWorks() {
  ...
  return <section id="how-it-works" className="bg-white/[.04] px-4 py-24 sm:px-6 lg:py-32"><div className="mx-auto max-w-7xl"><SectionHeading eyebrow="How it works" title="Three steps between idea and execution." copy="Simple on the surface. Durable, accountable automation underneath."/><div className="relative mt-16 grid gap-7 lg:grid-cols-3"><motion.div initial={{ scaleX: 0 }} whileInView={{ scaleX: 1 }} viewport={{ once: true }} transition={{ duration: 1.1 }} className="absolute left-[15%] right-[15%] top-7 hidden h-px origin-left bg-gradient-to-r from-[#D6BBFB] via-[#6941C6] to-[#D6BBFB] lg:block"/>{steps.map(([number,title,copy]) => <article key={number} className="relative"><span className="relative z-10 mx-auto grid size-14 place-items-center rounded-full border-4 border-white bg-[#6941C6] text-lg font-black text-white shadow-[0_8px_24px_rgba(105,65,198,.25)]">{number}</span><div className="mt-7 rounded-2xl border border-white/[.10] bg-white/[.04] p-5 shadow-[0_12px_32px_rgba(16,24,40,.08)]"><div className="aspect-video rounded-xl border border-white/[.10] bg-[#0A0F1E] p-4"><div className="flex gap-1.5"><i className="size-2 rounded-full bg-[#F97066]"/><i className="size-2 rounded-full bg-[#FDB022]"/><i className="size-2 rounded-full bg-[#32D583]"/></div><div className="mt-5 space-y-2"><i className="block h-2 w-4/5 rounded bg-[#E9D7FE]"/><i className="block h-2 w-3/5 rounded bg-[#EAECF0]"/><i className="block h-8 w-24 rounded-lg bg-[#6941C6]/90"/></div></div><h3 className="mt-6 text-xl font-black text-white">{title}</h3><p className="mt-3 leading-7 text-slate-400">{copy}</p></div></article>)}</div></div></section>
}
...
function InteractiveDemo() {
  ...
  return <section id="live-demo" className="relative overflow-hidden bg-[#0A0F1E] px-4 py-24 sm:px-6 lg:py-32"><div className="future-mesh pointer-events-none absolute inset-0 opacity-55"/><FutureOrbit/><div className="relative mx-auto grid max-w-7xl items-center gap-12 lg:grid-cols-[.85fr_1.15fr]"><SectionHeading align="left" eyebrow="Live demo" title="Feel how calm automation can be." copy="Click through the miniature Command Centre. Alpha asks for one decision, waits, then moves forward."/><div className="rounded-[24px] border border-[#D6BBFB] bg-white/[.04] p-3 shadow-[0_30px_80px_rgba(105,65,198,.14)] backdrop-blur-xl"><div className="relative min-h-[420px] overflow-hidden rounded-[18px] border border-white/[.10] bg-white/[.04] p-5 text-white sm:p-8">
    <div className="flex items-center justify-between"><span className="flex items-center gap-2 text-xs font-black text-violet-300"><Bot size={17}/>ALPHA COMMAND PALETTE</span><span className="rounded-lg border border-white/[.10] bg-[#0A0F1E] px-2 py-1 text-[10px] font-black text-slate-400">⌘ K</span></div>
    <div className="mt-4 rounded-xl border border-white/[.10] bg-[#0A0F1E] px-4 py-3 text-xs font-bold text-[#98A2B3]">Ask Alpha to automate anything…</div>
...
function Pricing() {
  ...
  return <section id="pricing" className="bg-white/[.04] px-4 py-24 sm:px-6 lg:py-32"><div className="mx-auto max-w-7xl"><SectionHeading eyebrow="Simple pricing" title="Start small. Scale to millions." copy="Planning is free. Pay only when Alpha works."/>
    <div className="mx-auto mt-8 flex w-fit rounded-xl border border-white/[.10] bg-[#0A0F1E] p-1"><button onClick={() => setYearly(false)} className={`min-h-11 rounded-lg px-5 text-sm font-bold ${!yearly ? 'bg-white/[.04] text-white shadow-sm' : 'text-slate-400'}`}>Monthly</button><button onClick={() => setYearly(true)} className={`min-h-11 rounded-lg px-5 text-sm font-bold ${yearly ? 'bg-white/[.04] text-white shadow-sm' : 'text-slate-400'}`}>Yearly <span className="text-[#039855]">−20%</span></button></div>
...
function MobileStickyCTA() {
  ...
  return <AnimatePresence>{visible && <motion.div initial={{ y: 90, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: 90, opacity: 0 }} transition={{ type: 'spring', stiffness: 240, damping: 24 }} className="fixed inset-x-0 bottom-0 z-40 border-t border-white/60 bg-white/[.04] p-3 backdrop-blur-2xl md:hidden"><Link to={user ? '/dashboard' : '/auth'} className="mx-auto flex min-h-12 max-w-sm items-center justify-center gap-2 rounded-full bg-[#6941C6] px-6 font-black text-white shadow-[0_14px_35px_rgba(105,65,198,.28)]">Start Automating — Free <ArrowRight size={17}/></Link></motion.div>}</AnimatePresence>
}

export default function Landing() {
  return <div className="min-h-screen overflow-x-hidden bg-white/[.04] font-sans text-white">
