...
function DotGrid() {
  return <div aria-hidden className="premium-grid pointer-events-none absolute inset-0 opacity-50" />
}
...
function Hero() {
  const { user } = useAuth()
  return <section className="relative overflow-hidden bg-[#FAFBFF] px-4 pb-28 pt-32 sm:px-6 lg:pb-36 lg:pt-40">
    <DotGrid/><div className="pointer-events-none absolute left-[8%] top-28 size-[360px] rounded-full bg-[#D6BBFB]/35 blur-[110px]"/><div className="pointer-events-none absolute right-[5%] top-44 size-[280px] rounded-full bg-[#B2DDFF]/30 blur-[100px]"/>
    <div className="relative mx-auto grid max-w-7xl items-center gap-16 lg:grid-cols-[1.02fr_.98fr]">
      <motion.div initial="hidden" animate="visible" variants={{ visible: { transition: { staggerChildren: .09 } } }} className="text-center lg:text-left">
        ...
        <motion.h1 variants={fadeUp} className="mt-7 text-[36px] font-black leading-[1.02] tracking-[-.045em] text-[#0B0F19] sm:text-6xl lg:text-[72px]">Delegate the work.<br/>Turn Your Idea Into <span className="reality-underline relative whitespace-nowrap text-[#0B0F19]">Reality</span></motion.h1>
...
function InteractiveDemo() {
  ...
  const choose = (answer: string) => { setAnswers(current => [...current, answer]); setStep(value => Math.min(value + 1, choices.length)) }
  const reset = () => { setStep(0); setAnswers([]) }
  return <section id="live-demo" className="bg-[#0B0F19] px-4 py-24 text-white sm:px-6 lg:py-32"><div className="mx-auto grid max-w-7xl items-center gap-12 lg:grid-cols-[.85fr_1.15fr]"><SectionHeading align="left" eyebrow="Live demo" title="Feel how calm automation can be." copy="Click through the miniature Command Centre. Alpha asks for one decision, waits, then moves forward."/><div className="rounded-[24px] border border-white/10 bg-white/[.06] p-3 shadow-[0_30px_80px_rgba(0,0,0,.35)] backdrop-blur"><div className="min-h-[360px] rounded-[18px] bg-white p-5 text-[#0B0F19] sm:p-8"><div className="flex items-center justify-between"><span className="flex items-center gap-2 text-xs font-black text-[#6941C6]"><Bot size={17}/>ALPHA</span><button onClick={reset} className="flex min-h-11 items-center gap-2 rounded-xl px-3 text-xs font-bold text-[#667085] hover:bg-[#F9FAFB]"><TimerReset size={15}/>Restart</button></div><div className="mt-8 space-y-3">{answers.map((answer,index) => <div key={`${answer}-${index}`} className="flex justify-end"><span className="rounded-2xl rounded-br-sm bg-[#6941C6] px-4 py-3 text-sm font-semibold text-white">{answer}</span></div>)}</div><AnimatePresence mode="wait">{step < choices.length ? <motion.div key={step} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} className="mt-6"><p className="max-w-md text-xl font-black leading-8">{choices[step].question}</p><div className="mt-5 flex flex-wrap gap-2">{choices[step].answers.map(answer => <button key={answer} onClick={() => choose(answer)} className="min-h-11 rounded-xl border border-[#D6BBFB] bg-[#F4F3FF] px-4 text-sm font-bold text-[#6941C6] transition hover:bg-[#E9D7FE]">{answer}</button>)}</div></motion.div> : <motion.div initial={{ opacity: 0, scale: .97 }} animate={{ opacity: 1, scale: 1 }} className="mt-8 rounded-2xl border border-[#ABEFC6] bg-[#ECFDF3] p-5"><p className="flex items-center gap-2 font-black text-[#027A48]"><CheckCircle2 size={19}/>Plan ready for your approval</p><p className="mt-2 text-sm font-medium text-[#475467]">LinkedIn · Mon–Fri · 9am · Credits charged only after confirmed posts.</p></motion.div>}</AnimatePresence></div></div></div></section>
}
...
function Stats() {
  return <section className="bg-white px-4 py-24 sm:px-6"><motion.div initial={{ opacity: 0, scale: .98 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }} className="mx-auto max-w-7xl overflow-hidden rounded-[28px] bg-[#6941C6] px-6 py-14 text-center text-white shadow-[0_24px_60px_rgba(105,65,198,.25)] sm:px-12"><p className="text-sm font-bold uppercase tracking-[.18em] text-[#E9D7FE]">The opportunity</p><p className="mt-5 text-5xl font-black tracking-[-.05em] sm:text-7xl">$1.77 Trillion</p><h2 className="mt-4 text-2xl font-black sm:text-4xl">AI market projected by 2032. Be early.</h2><p className="mx-auto mt-4 max-w-2xl text-[#E9D7FE]">The advantage belongs to people who learn how to delegate outcomes—not just generate answers.</p></motion.div></section>
}
...
function TiltCard({ children, featured = false }: { children: ReactNode; featured?: boolean }) {
  const reduce = useReducedMotion()
  return <motion.article whileHover={reduce ? undefined : { y: -8, rotateX: 2, rotateY: -2, scale: 1.015 }} transition={{ type: 'spring', stiffness: 220, damping: 18 }} style={{ transformPerspective: 900 }} className={`relative rounded-2xl border bg-white p-6 shadow-[0_1px_2px_rgba(16,24,40,.05)] transition-shadow hover:shadow-[0_20px_45px_rgba(16,24,40,.13)] ${featured ? 'border-2 border-[#6941C6]' : 'border-[#EAECF0]'}`}>{children}</motion.article>
}
...
export default function Landing() {
  return <div className="min-h-screen overflow-x-hidden bg-white font-sans text-[#0B0F19]">
    ...
    <Header/><main><Hero/><LogoMarquee/><ProblemSolution/><HowItWorks/><InteractiveDemo/><AutomationGallery/><ActivityRail/><ConnectedApps/><FeatureDeepDive/><Stats/><Testimonials/><Pricing/><FAQ/><FinalCTA/></main><Footer/>
  </div>
}
