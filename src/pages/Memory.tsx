    ...
    <div className="min-h-screen px-5 py-8 md:px-10">
      <div className="mx-auto max-w-5xl">
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
          <section className="rounded-2xl border border-violet-400/20 bg-violet-500/10 p-5 backdrop-blur-2xl">
            ...
            <div className="mt-3 space-y-2">
              {memory.projects.slice(0, 10).map(p => (
                <div key={p.id} className="rounded-xl border border-violet-400/20 bg-violet-500/10 p-3">
                  <div className="text-sm font-medium">{p.title}</div>
                  <div className="mt-1 text-xs text-white/55">{p.goal}</div>
                  <div className="mt-2 flex flex-wrap gap-1">
                    {p.systems.map(s => <span key={s} className="rounded-md bg-violet-500/10 px-2 py-1 text-[10px] text-zinc-300">{s}</span>)}
                  </div>
                </div>
              ))}
            </div>
          ...
          <section className="rounded-2xl border border-violet-400/20 bg-violet-500/10 p-5 backdrop-blur-2xl">
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              {memory.architecture.map(a => <span key={a} className="flex items-center gap-1.5 rounded-lg bg-violet-500/10 px-3 py-2 text-xs text-zinc-300"><Database size={13} /> {a}</span>)}
            </div>
          </section>

          <section className="rounded-2xl border border-violet-400/20 bg-violet-500/10 p-5 backdrop-blur-2xl md:col-span-2">
  ...
  return (
    <div className="rounded-xl border border-violet-400/20 bg-violet-500/10 p-3">
      ...
      <div className="mt-2 flex items-center gap-2">
        <span className="h-6 w-6 rounded-full border border-violet-400/20" style={{ background: value }} />
