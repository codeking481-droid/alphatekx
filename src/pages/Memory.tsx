    ...
    <div className="min-h-screen px-5 py-8 md:px-10">
      <div className="mx-auto max-w-5xl">
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
          <section className="rounded-2xl border border-white/10 bg-white/[.04] p-5 backdrop-blur-2xl">
            ...
            <div className="mt-3 space-y-2">
              {memory.projects.slice(0, 10).map(p => (
                <div key={p.id} className="rounded-xl border border-white/[0.08] bg-white/[.04] p-3">
                  <div className="text-sm font-medium">{p.title}</div>
                  <div className="mt-1 text-xs text-white/55">{p.goal}</div>
                  <div className="mt-2 flex flex-wrap gap-1">
                    {p.systems.map(s => <span key={s} className="rounded-md bg-white/[.04] px-2 py-1 text-[10px] text-zinc-300">{s}</span>)}
                  </div>
                </div>
              ))}
            </div>
          ...
          <section className="rounded-2xl border border-white/10 bg-white/[.04] p-5 backdrop-blur-2xl">
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              {memory.architecture.map(a => <span key={a} className="flex items-center gap-1.5 rounded-lg bg-white/[.04] px-3 py-2 text-xs text-zinc-300"><Database size={13} /> {a}</span>)}
            </div>
          </section>

          <section className="rounded-2xl border border-white/10 bg-white/[.04] p-5 backdrop-blur-2xl md:col-span-2">
...
function ColorSwatch({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded-xl border border-white/[0.08] bg-white/[.04] p-3">
