...
export default function Workers() {
  ...
  return <main className="mx-auto min-h-screen max-w-6xl overflow-x-hidden px-5 py-12 sm:py-16">
    ...
    <div className="mt-8 grid min-w-0 gap-5 lg:grid-cols-[300px_minmax(0,1fr)]">
      <aside className="overflow-hidden rounded-xl border border-violet-400/20 liquid-glass"><div className="border-b border-violet-400/20 p-4 text-sm font-medium">Your workers</div>{workers.length === 0 ? <p className="p-5 text-sm text-white/55">No workers yet. Create one to begin.</p> : workers.map(worker => <button key={worker.id} onClick={() => setSelectedId(worker.id)} className={`flex w-full items-center gap-3 border-b border-violet-400/20 p-4 text-left last:border-0 ${selected?.id === worker.id ? 'bg-violet-500/10' : 'hover:bg-violet-500/10'}`}><span className="grid size-10 shrink-0 place-items-center rounded-lg btn-alpha text-white"><Bot size={18}/></span><span className="min-w-0"><strong className="block truncate text-sm">{worker.name}</strong><span className="text-xs capitalize text-white/55">{worker.role} / {worker.provider || 'groq'}</span></span></button>)}</aside>
      <section className="min-w-0 rounded-xl border border-violet-400/20 liquid-glass p-4 sm:p-5">{selected ? <><div className="flex flex-wrap items-start justify-between gap-3"><div><h2 className="font-semibold">{selected.name}</h2><p className="mt-1 text-sm text-white/55">{selected.purpose}</p></div><div className="flex items-center gap-2"><span className="rounded-full bg-violet-500/10 px-3 py-1.5 text-xs capitalize">{selected.provider || 'groq'}</span><button onClick={() => void remove()} className="grid size-11 place-items-center rounded-lg border border-violet-400/20" aria-label="Delete worker"><Trash2 size={16}/></button></div></div>
        <div className="mt-5 min-h-72 space-y-3 rounded-xl bg-violet-500/10 p-3 sm:p-5">{selected.memory.length ? selected.memory.map((message, index) => { const fromUser = message.startsWith('User:'); return <div key={`${index}-${message.slice(0, 12)}`} className={`flex ${fromUser ? 'justify-end' : 'justify-start'}`}><p className={`max-w-[90%] whitespace-pre-wrap break-words rounded-xl px-4 py-3 text-sm leading-6 ${fromUser ? 'btn-alpha text-white' : 'border border-violet-400/20 liquid-glass'}`}>{message.replace(/^(User|Worker):\s*/, '')}</p></div> }) : <div className="grid min-h-60 place-items-center text-center"><div><Sparkles className="mx-auto text-white/35"/><p className="mt-3 text-sm text-white/55">Ask your worker to complete a focused task.</p></div></div>}</div>
        {notice && <p role="status" className="mt-4 rounded-lg bg-violet-500/10 p-3 text-sm text-white/80">{notice}</p>}
        <div className="mt-4 flex items-end gap-2 rounded-xl border border-violet-400/20 p-2 focus-within:border-[#E56B2D]"><textarea value={prompt} onChange={event => setPrompt(event.target.value)} onKeyDown={event => { if (event.key === 'Enter' && !event.shiftKey) { event.preventDefault(); void run() } }} className="min-h-20 min-w-0 flex-1 resize-none px-2 py-2 text-sm outline-none" placeholder={`Ask ${selected.name}...`}/><button onClick={() => void run()} disabled={running || !prompt.trim()} className="grid size-11 shrink-0 place-items-center rounded-lg btn-alpha text-white disabled:opacity-30" aria-label="Send task">{running ? <LoaderCircle className="animate-spin" size={17}/> : <ArrowUp size={17}/>}</button></div>
