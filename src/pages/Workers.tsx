...
export default function Workers() {
  ...
  return <main className="mx-auto min-h-screen max-w-6xl overflow-x-hidden px-5 py-12 sm:py-16">
    <header className="flex flex-wrap items-end justify-between gap-4"><div><h1 className="text-2xl font-semibold">AI Workers</h1><p className="mt-2 text-sm text-gray-500">Create a specialist that follows your instructions and remembers recent conversations.</p></div><button onClick={() => setShowCreate(true)} className="flex min-h-11 items-center gap-2 rounded-lg bg-black px-4 text-sm text-white"><Plus size={17}/>Create worker</button></header>
    <div className="mt-8 grid min-w-0 gap-5 lg:grid-cols-[300px_minmax(0,1fr)]">
      <aside className="overflow-hidden rounded-xl border border-gray-200 bg-white"><div className="border-b border-gray-100 p-4 text-sm font-medium">Your workers</div>{workers.length === 0 ? <p className="p-5 text-sm text-gray-500">No workers yet. Create one to begin.</p> : workers.map(worker => <button key={worker.id} onClick={() => setSelectedId(worker.id)} className={`flex w-full items-center gap-3 border-b border-gray-100 p-4 text-left last:border-0 ${selected?.id === worker.id ? 'bg-gray-100' : 'hover:bg-gray-50'}`}><span className="grid size-10 shrink-0 place-items-center rounded-lg bg-black text-white"><Bot size={18}/></span><span className="min-w-0"><strong className="block truncate text-sm">{worker.name}</strong><span className="text-xs capitalize text-gray-500">{worker.role} / {worker.provider || 'groq'}</span></span></button>)}</aside>
      <section className="min-w-0 rounded-xl border border-gray-200 bg-white p-4 sm:p-5">{selected ? <><div className="flex flex-wrap items-start justify-between gap-3"><div><h2 className="font-semibold">{selected.name}</h2><p className="mt-1 text-sm text-gray-500">{selected.purpose}</p></div><div className="flex items-center gap-2"><span className="rounded-full bg-gray-100 px-3 py-1.5 text-xs capitalize">{selected.provider || 'groq'}</span><button onClick={() => void remove()} className="grid size-11 place-items-center rounded-lg border border-gray-300" aria-label="Delete worker"><Trash2 size={16}/></button></div></div>
        <div className="mt-5 min-h-72 space-y-3 rounded-xl bg-gray-50 p-3 sm:p-5">{selected.memory.length ? selected.memory.map((message, index) => { const fromUser = message.startsWith('User:'); return <div key={`${index}-${message.slice(0, 12)}`} className={`flex ${fromUser ? 'justify-end' : 'justify-start'}`}><p className={`max-w-[90%] whitespace-pre-wrap break-words rounded-xl px-4 py-3 text-sm leading-6 ${fromUser ? 'bg-black text-white' : 'border border-gray-200 bg-white'}`}>{message.replace(/^(User|Worker):\s*/, '')}</p></div> }) : <div className="grid min-h-60 place-items-center text-center"><div><Sparkles className="mx-auto text-gray-300"/><p className="mt-3 text-sm text-gray-500">Ask your worker to complete a focused task.</p></div></div>}</div>
        {notice && <p role="status" className="mt-4 rounded-lg bg-gray-50 p-3 text-sm text-gray-700">{notice}</p>}
        <div className="mt-4 flex items-end gap-2 rounded-xl border border-gray-300 p-2 focus-within:border-black"><textarea value={prompt} onChange={event => setPrompt(event.target.value)} onKeyDown={event => { if (event.key === 'Enter' && !event.shiftKey) { event.preventDefault(); void run() } }} className="min-h-20 min-w-0 flex-1 resize-none px-2 py-2 text-sm outline-none" placeholder={`Ask ${selected.name}...`}/><button onClick={() => void run()} disabled={running || !prompt.trim()} className="grid size-11 shrink-0 place-items-center rounded-lg bg-black text-white disabled:opacity-30" aria-label="Send task">{running ? <LoaderCircle className="animate-spin" size={17}/> : <ArrowUp size={17}/>}</button></div>
        <Link to="/settings/api-keys" className="mt-4 inline-flex min-h-11 items-center gap-2 text-sm font-medium"><KeyRound size={16}/>Manage provider keys</Link></> : <div className="grid min-h-96 place-items-center text-sm text-gray-500">Create a worker to begin.</div>}</section>
    </div>
    {showCreate && <div className="fixed inset-0 z-[70] grid place-items-center overflow-y-auto bg-black/30 p-4" onMouseDown={() => !creating && setShowCreate(false)}><section className="w-full max-w-lg rounded-xl bg-white p-6" onMouseDown={event => event.stopPropagation()}><h2 className="text-xl font-semibold">Create an AI worker</h2><p className="mt-2 text-sm text-gray-500">Choose its job, provider, and clear operating instructions.</p><div className="mt-5 grid gap-4"><Field label="Name"><input value={form.name} onChange={event => setForm({ ...form, name: event.target.value })} className="control" placeholder="Research Assistant"/></Field><div className="grid gap-4 sm:grid-cols-2"><Field label="Role"><select value={form.role} onChange={event => setForm({ ...form, role: event.target.value as WorkerRole })} className="control">{roles.map(role => <option key={role}>{role}</option>)}</select></Field><Field label="AI provider"><select value={form.provider} onChange={event => setForm({ ...form, provider: event.target.value as keyof typeof defaultModels })} className="control">{Object.keys(defaultModels).map(provider => <option key={provider}>{provider}</option>)}</select></Field></div><Field label="Purpose"><input value={form.purpose} onChange={event => setForm({ ...form, purpose: event.target.value })} className="control" placeholder="Research markets and summarize reliable sources"/></Field><Field label="Instructions"><textarea value={form.instructions} onChange={event => setForm({ ...form, instructions: event.target.value })} className="control min-h-28 resize-none" placeholder="Be concise, cite sources, and state uncertainty."/></Field></div>{notice && <p className="mt-4 rounded-lg bg-gray-50 p-3 text-sm">{notice}</p>}<div className="mt-6 flex justify-end gap-2"><button onClick={() => setShowCreate(false)} disabled={creating} className="min-h-11 rounded-lg border border-gray-300 px-4 text-sm">Cancel</button><button onClick={() => void create()} disabled={creating} className="flex min-h-11 items-center gap-2 rounded-lg bg-black px-4 text-sm text-white disabled:opacity-50">{creating && <LoaderCircle className="animate-spin" size={16}/>}Create worker</button></div></section></div>}
  </main>
}
