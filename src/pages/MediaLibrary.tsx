...
export default function MediaLibrary() {
  ...
  return <main className="mx-auto min-h-[calc(100dvh-8rem)] w-full max-w-6xl bg-white px-4 py-8 text-slate-900 sm:px-6">
    <header className="flex flex-wrap items-end justify-between gap-4">
      <div><p className="text-xs font-black uppercase tracking-[.18em] text-[#6D28D9]">Content vault</p><h1 className="mt-2 text-3xl font-black">Your Media Library</h1><p className="mt-2 max-w-2xl text-sm font-semibold text-slate-600">Upload up to 20 videos at once. Alpha keeps the files private and prepares an honest publishing queue.</p></div>
    ...
    <section className="mt-7 grid grid-cols-2 gap-3 lg:grid-cols-4">
      {Object.entries(counts).map(([label, value]) => <article key={label} className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm"><p className="text-xs font-black uppercase tracking-wide text-slate-500">{label}</p><p className="mt-2 text-3xl font-black">{value}</p></article>)}
    </section>

    {notice && <div role="status" className="mt-5 rounded-xl border border-violet-200 bg-violet-50 p-3 text-sm font-bold text-slate-800">{notice}</div>}
    ...
    >
      ...
      <span className="mt-1 block text-xs font-semibold text-slate-500">MP4, WebM, MOV, JPEG, PNG or WebP · 500MB maximum per file</span>
    </button>
    ...
    {loading ? <section className="mt-7 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{[1,2,3].map(value => <div key={value} className="h-64 animate-pulse rounded-2xl bg-slate-100"/>)}</section> :
      items.length === 0 ? <section className="mt-7 rounded-2xl border border-slate-200 bg-slate-50 p-10 text-center"><FileVideo2 className="mx-auto text-slate-400"/><h2 className="mt-3 font-black">Your vault is ready</h2><p className="mt-1 text-sm font-semibold text-slate-500">Upload your first file to begin.</p></section> :
      <section className="mt-7 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{items.map(item => <article key={item.id} className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
        <div className="relative grid aspect-video place-items-center bg-slate-100">
