...
export default function MediaLibrary() {
  ...
  return <main className="mx-auto min-h-[calc(100dvh-8rem)] w-full max-w-6xl bg-white/[.04] px-4 py-8 text-white sm:px-6">
    ...
    <section className="mt-7 grid grid-cols-2 gap-3 lg:grid-cols-4">
      {Object.entries(counts).map(([label, value]) => <article key={label} className="rounded-2xl border border-white/[.10] bg-white/[.04] p-4 shadow-sm"><p className="text-xs font-black uppercase tracking-wide text-slate-400">{label}</p><p className="mt-2 text-3xl font-black">{value}</p></article>)}
    </section>
    ...
    {loading ? <section className="mt-7 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{[1,2,3].map(value => <div key={value} className="h-64 animate-pulse rounded-2xl bg-white/[.05]"/>)}</section> :
      items.length === 0 ? <section className="mt-7 rounded-2xl border border-white/[.10] bg-white/[.05] p-10 text-center"><FileVideo2 className="mx-auto text-slate-400"/><h2 className="mt-3 font-black">Your vault is ready</h2><p className="mt-1 text-sm font-semibold text-slate-400">Upload your first file to begin.</p></section> :
      <section className="mt-7 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{items.map(item => <article key={item.id} className="overflow-hidden rounded-2xl border border-white/[.10] bg-white/[.04] shadow-sm">
        <div className="relative grid aspect-video place-items-center bg-white/[.05]">
          ...
          <span className={`absolute right-3 top-3 rounded-full border px-2.5 py-1 text-[10px] font-black uppercase ${item.status === 'published' ? 'border-emerald-200 bg-emerald-50 text-emerald-300' : item.status === 'scheduled' ? 'border-violet-200 bg-violet-500/10 text-violet-200' : 'border-white/[.10] bg-white/[.04] text-slate-400'}`}>{item.status}</span>
        </div>
        <div className="p-4"><h2 className="truncate font-black">{item.title || item.file_name}</h2><p className="mt-1 text-xs font-semibold text-slate-400">{(item.file_size / 1048576).toFixed(1)}MB {item.scheduled_for ? `· ${new Date(item.scheduled_for).toLocaleString()}` : ''}</p><div className="mt-4 flex flex-wrap gap-2">{item.file_url && <a href={item.file_url} target="_blank" rel="noreferrer" className="action bg-white/[.04] text-slate-400"><Play size={15}/>Preview</a>}{item.file_type === 'video' && ['ready', 'failed'].includes(item.status) && <button onClick={() => void publishNow(item)} disabled={busy} className="action bg-[#6D28D9] text-white"><UploadCloud size={15}/>Publish now</button>}<button onClick={() => void remove(item)} disabled={busy} className="action bg-white/[.04] text-rose-300"><Trash2 size={15}/>Delete</button></div></div>
      </article>)}</section>}

    {scheduleOpen && <div className="fixed inset-0 z-50 grid place-items-end bg-slate-950/35 sm:place-items-center" onClick={() => setScheduleOpen(false)}><section className="w-full rounded-t-3xl bg-white/[.04] p-6 text-white shadow-2xl sm:max-w-md sm:rounded-3xl" onClick={event => event.stopPropagation()}><div className="flex items-center justify-between"><div><h2 className="text-xl font-black">Daily YouTube queue</h2><p className="mt-1 text-sm font-semibold text-slate-400">One ready video per day · Africa/Lagos</p></div><button onClick={() => setScheduleOpen(false)} className="grid size-10 place-items-center rounded-full bg-white/[.05]"><X size={18}/></button></div><div className="mt-6 grid gap-4"><label className="text-sm font-bold">Start date<input type="date" value={startDate} min={new Date().toISOString().slice(0,10)} onChange={event => setStartDate(event.target.value)} className="mt-1 h-12 w-full rounded-xl border border-white/[.10] px-3 text-white"/></label><label className="text-sm font-bold">Time<input type="time" value={time} onChange={event => setTime(event.target.value)} className="mt-1 h-12 w-full rounded-xl border border-white/[.10] px-3 text-white"/></label><div className="rounded-xl border border-emerald-200 bg-emerald-50 p-3 text-xs font-bold text-emerald-300"><CheckCircle2 className="mr-1 inline" size={15}/>No credits are charged while scheduling. Each confirmed publication costs one credit.</div><button onClick={() => void scheduleReady()} disabled={busy} className="min-h-12 rounded-xl bg-[#6D28D9] px-5 font-black text-white disabled:opacity-50">{busy ? 'Scheduling…' : 'Confirm queue'}</button></div></section></div>}
  </main>
}
