      ...
      <section className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {releasedPlatforms.map(item => {
          ...
          return <article id={`platform-${item.id}`} key={item.id} className="liquid-glass rounded-[20px] p-5">
            <div className="flex items-center gap-3"><span className="grid size-12 place-items-center rounded-full text-white" style={{ background: item.id === 'instagram' ? 'linear-gradient(135deg,#F58529,#DD2A7B,#8134AF)' : item.color }}><ConnectorIcon connector={connector}/></span><div><h2 className="font-black">{item.name}</h2><span className={`mt-1 inline-flex rounded-full border px-2 py-1 text-[11px] font-bold ${connected ? 'border-emerald-300/25 bg-emerald-300/10 text-emerald-200' : 'border-white/10 bg-white/5 text-slate-300'}`}>{connected ? '● Connected' : '○ Not connected'}</span></div></div>
