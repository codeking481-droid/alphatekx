...
function GuidedCommandCentre({ platform, creating, notice, onComplete, onBack }: { platform: string; creating: boolean; notice: string; onComplete: (message: string) => void; onBack: () => void }) {
  ...
  return (
    <main className="min-h-[calc(100dvh-8rem)] bg-white/[.04] px-4 py-8 sm:px-6 lg:min-h-[calc(100dvh-4rem)] lg:py-12">
      <div className="mx-auto max-w-5xl">
        ...
        {step < 5 ? (
          <section className="mx-auto mt-10 max-w-3xl rounded-[1.75rem] border border-white/[.10] bg-white/[.04] p-6 shadow-[0_30px_75px_rgba(15,23,42,.14)] sm:p-9">
            ...
            <div className="mt-3 h-2 overflow-hidden rounded-full bg-white/[.05]"><div className="h-full rounded-full bg-[#6D28D9] transition-all" style={{ width: `${((step + 1) / 5) * 100}%` }}/></div>
            ...
            <div className="mt-8 grid gap-3 sm:grid-cols-2">
              ...
              {step === 1 && ['Mon–Fri', 'Every day', 'Custom'].map(value => <button key={value} onClick={() => value === 'Custom' ? setCustom('') : choose('days', value)} className="min-h-14 rounded-xl border-2 border-white/[.10] bg-white/[.04] font-black text-white transition hover:border-violet-300 hover:bg-violet-500/10">{value}{value === 'Mon–Fri' && <span className="ml-2 text-xs text-violet-300">Suggested</span>}</button>)}
              {step === 2 && <><button onClick={() => choose('time', '9:00 AM')} className="min-h-14 rounded-xl bg-[#6D28D9] font-black text-white shadow-lg shadow-violet-200">Yes, use 9am</button><button onClick={() => setCustom('')} className="min-h-14 rounded-xl border-2 border-white/[.10] bg-white/[.04] font-black text-white">Choose different</button></>}
              {step === 3 && ['7 days', '14 days', '30 days', 'Custom'].map(value => <button key={value} onClick={() => value === 'Custom' ? setCustom('') : choose('duration', value)} className="min-h-14 rounded-xl border-2 border-white/[.10] bg-white/[.04] font-black text-white transition hover:border-violet-300 hover:bg-violet-500/10">{value}</button>)}
          ...
          <section className="mx-auto mt-10 max-w-4xl overflow-hidden rounded-[1.75rem] border border-white/[.10] bg-white/[.04] shadow-[0_30px_75px_rgba(15,23,42,.14)]">
            ...
            <div className="grid gap-7 p-6 sm:p-9 lg:grid-cols-[.85fr_1.15fr]">
              <div className="grid content-start gap-3">
                {[['Platform', plan.platform], ['Days', plan.days], ['Time', plan.time], ['Duration', plan.duration], ['Total posts', String(totalPosts)], ['Estimated cost', `${estimatedCredits} credits`]].map(([label, value]) => <div key={label} className="flex items-center justify-between rounded-xl bg-white/[.05] px-4 py-3"><span className="text-sm font-bold text-slate-400">{label}</span><span className="text-sm font-black text-white">{value}</span></div>)}
              </div>
              <div><p className="text-xs font-black uppercase tracking-[.16em] text-violet-300">Two sample directions</p><div className="mt-3 space-y-3">{sampleCaptions.map((caption, index) => <div key={caption} className="rounded-xl border border-white/[.10] bg-white/[.04] p-4 shadow-sm"><p className="text-xs font-black text-violet-400">SAMPLE {index + 1}</p><p className="mt-2 text-sm font-semibold leading-6 text-slate-400">{caption}</p></div>)}</div><p className="mt-3 text-xs font-bold text-slate-400">Alpha checks the last 10 posts before generating each live caption.</p></div>
            </div>
            {notice && <p className="mx-6 mb-4 rounded-xl border border-rose-200 bg-rose-50 p-3 text-sm font-bold text-rose-300 sm:mx-9">{notice}</p>}
            <div className="flex flex-col gap-3 border-t border-white/[.10] bg-white/[.05] p-6 sm:flex-row sm:justify-end sm:p-7"><button onClick={() => { setStep(0); setEditing(!editing) }} className="inline-flex min-h-13 items-center justify-center gap-2 rounded-xl border-2 border-white/[.10] bg-white/[.04] px-6 font-black text-white"><Edit3 size={17}/>Edit</button><button onClick={approve} disabled={creating} className="inline-flex min-h-13 items-center justify-center gap-2 rounded-xl bg-[#6D28D9] px-7 font-black text-white shadow-[0_14px_32px_rgba(109,40,217,.28)] disabled:opacity-50">{creating ? <LoaderCircle className="animate-spin" size={18}/> : <><CalendarDays size={18}/>Approve & Schedule</>}</button></div>
          </section>
        )}
