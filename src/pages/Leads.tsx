...
export default function Leads() {
  ...
  return <main className="mx-auto min-h-[calc(100dvh-8rem)] w-full max-w-6xl bg-white/[.04] px-4 py-8 text-white sm:px-6">
    ...
    {loading ? <div className="mt-8 grid min-h-64 place-items-center rounded-2xl bg-white/[.05]"><LoaderCircle className="animate-spin text-violet-300"/></div> :
      leads.length === 0 ? <section className="mt-8 rounded-2xl border border-dashed border-white/[.10] bg-white/[.05] p-10 text-center"><Users className="mx-auto text-slate-400"/><h2 className="mt-3 font-black">No captured leads yet</h2><p className="mt-1 text-sm font-semibold text-slate-400">Leads appear only after a person responds to a clear opt-in CTA. Alpha will never invent leads or send unapproved messages.</p></section> :
      <section className="mt-8 overflow-hidden rounded-2xl border border-white/[.10] bg-white/[.04] shadow-sm"><div className="overflow-x-auto"><table className="w-full min-w-[850px] text-left text-sm"><thead className="bg-white/[.05] text-xs uppercase text-slate-400"><tr><th className="px-5 py-4">Lead</th><th className="px-5 py-4">Platform</th><th className="px-5 py-4">Comment</th><th className="px-5 py-4">DM</th><th className="px-5 py-4">Value</th><th className="px-5 py-4">Status</th></tr></thead><tbody>{leads.map(lead => <tr key={lead.id} className="border-t border-white/[.10]"><td className="px-5 py-4"><p className="font-black">{lead.lead_name || lead.lead_handle || 'Platform user'}</p><p className="mt-1 text-xs font-semibold text-slate-400">{lead.lead_handle ? `@${lead.lead_handle.replace(/^@/, '')}` : new Date(lead.created_at).toLocaleDateString()}</p></td><td className="px-5 py-4 font-bold capitalize">{lead.platform}</td><td className="max-w-sm px-5 py-4 font-semibold text-slate-400"><span className="line-clamp-2">{lead.comment_text}</span></td><td className="px-5 py-4">{lead.dm_sent ? <span className="font-bold text-emerald-300">Confirmed sent</span> : <span className="font-bold text-amber-300">Not sent</span>}</td><td className="px-5 py-4 font-bold">{money.format(lead.estimated_value || 0)}</td><td className="px-5 py-4"><select value={lead.status} disabled={busy === lead.id} onChange={event => void changeStatus(lead, event.target.value as LeadStatus)} className="h-10 rounded-lg border border-white/[.10] bg-white/[.04] px-3 font-bold text-white">{STATUSES.map(status => <option key={status} value={status}>{status[0].toUpperCase() + status.slice(1)}</option>)}</select></td></tr>)}</tbody></table></div></section>}
  </main>
}

function Stat({ icon: Icon, label, value }: { icon: typeof Users; label: string; value: string }) {
  return <article className="rounded-2xl border border-white/[.10] bg-white/[.04] p-4 shadow-sm"><Icon size={18} className="text-violet-300"/><p className="mt-4 text-2xl font-black">{value}</p><p className="mt-1 text-xs font-black uppercase tracking-wide text-slate-400">{label}</p></article>
}
