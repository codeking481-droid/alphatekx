...
export default function Home() {
  ...
  return <div className="mx-auto flex min-h-screen w-full max-w-[820px] min-w-0 flex-col overflow-x-hidden px-4 pb-32 pt-20 sm:px-6">
    ...
    <div className="fixed bottom-0 left-0 right-0 bg-gradient-to-t from-[#0A0A0A] via-[#0A0A0A] to-transparent px-4 pb-5 pt-8"><div className="mx-auto flex max-w-[800px] items-end gap-2 rounded-2xl border border-white/[.15] liquid-glass p-2 shadow-sm"><textarea value={input} onChange={event => setInput(event.target.value)} onKeyDown={event => { if (event.key === 'Enter' && !event.shiftKey) { event.preventDefault(); void send() } }} className="max-h-40 min-h-12 flex-1 resize-none px-3 py-3 text-sm outline-none" placeholder="Ask Alpha anything..."/><button onClick={() => void send()} disabled={!input.trim() || pending} className="grid size-11 shrink-0 place-items-center rounded-full btn-alpha text-white disabled:opacity-30" aria-label="Send"><ArrowUp size={18}/></button></div></div>
  </div>
}
...
function ClockCard() {
  ...
  return <section className="rounded-xl border border-white/[.12] liquid-glass p-5 shadow-sm"><p className="text-xs text-white/55">{Intl.DateTimeFormat(undefined, { dateStyle: 'full' }).format(now)}</p><p className="mt-2 text-4xl font-semibold tabular-nums">{Intl.DateTimeFormat(undefined, { hour: '2-digit', minute: '2-digit', second: '2-digit' }).format(now)}</p><p className="mt-2 text-xs text-white/55">{Intl.DateTimeFormat().resolvedOptions().timeZone}</p></section>
}
...
function CurrencyCard({ initial }: { initial?: CurrencyResult }) {
  ...
  return <section className="rounded-xl border border-white/[.12] liquid-glass p-5 shadow-sm"><div className="grid gap-3 sm:grid-cols-[1fr_110px_110px_auto]"><input type="number" min="0" value={amount} onChange={event => setAmount(event.target.value)} className="min-h-11 rounded-lg border border-white/[.15] px-3" aria-label="Amount"/>{[from,to].map((value,index)=><select key={index} value={value} onChange={event => index === 0 ? setFrom(event.target.value) : setTo(event.target.value)} className="min-h-11 rounded-lg border border-white/[.15] px-2" aria-label={index === 0 ? 'From currency' : 'To currency'}>{currencies.map(code=><option key={code}>{code}</option>)}</select>)}<button onClick={() => void convert()} disabled={loading || !Number(amount)} className="flex min-h-11 items-center justify-center gap-2 rounded-lg btn-alpha px-4 text-sm text-white disabled:opacity-40">{loading?<LoaderCircle size={16} className="animate-spin"/>:<RefreshCw size={16}/>}Convert</button></div>{result&&<p className="mt-5 text-2xl font-semibold">{result.amount.toLocaleString()} {result.from} = {result.result.toLocaleString(undefined,{maximumFractionDigits:2})} {result.to}</p>}{error&&<p className="mt-3 text-sm text-red-600">{error}</p>}</section>
}
