...
export default function FoundersLegacy() {
  return <main className="min-h-screen bg-white/[.04] px-4 py-8 text-white sm:px-6 lg:py-16">
    <div className="mx-auto max-w-4xl">
      ...
      <section className="mt-8 overflow-hidden rounded-[32px] border border-white/[.10] bg-[#0A0F1E] p-6 shadow-[0_30px_80px_rgba(15,23,42,.12)] sm:p-10 lg:p-14">
        ...
        <div className="mt-10 grid gap-4 sm:grid-cols-2">{milestones.map(([title, copy], index) => <article key={title} className="rounded-2xl border border-white/[.10] bg-white/[.04] p-5 shadow-sm"><span className="grid size-10 place-items-center rounded-xl bg-violet-500/10 font-black text-violet-300">{index + 1}</span><h2 className="mt-4 text-lg font-black">{title}</h2><p className="mt-2 text-sm font-semibold leading-6 text-slate-400">{copy}</p></article>)}</div>
