...
export default function Builder() {
  ...
  if (!mission) return <div className="grid min-h-screen place-items-center bg-background p-6 text-center"><div><h1 className="text-xl font-semibold">Mission not found</h1><Link to="/workspace" className="mt-5 inline-flex rounded-lg bg-violet-500/10 px-5 py-3 text-sm text-white transition-all hover:bg-blue-500/10">Return to dashboard</Link></div></div>

  return (
    <div className="flex h-full min-h-0 min-w-0 flex-col overflow-hidden bg-background">
      <header className="flex min-h-16 flex-wrap items-center justify-between gap-3 border-b border-violet-400/20 bg-violet-500/10 px-5 py-3 ">
        ...
        <div className="flex max-w-full flex-1 items-center justify-end gap-2 overflow-x-auto">
          {tabs.map(t => (
            <button
              key={t.id}
              onClick={() => setTab(t.id)}
              className={`flex min-h-9 flex-shrink-0 items-center gap-1.5 rounded-lg px-3 text-xs font-medium transition-all ${tab === t.id ? 'bg-violet-500/10 text-white' : 'border border-violet-400/20 bg-violet-500/10 text-zinc-300 hover:bg-violet-500/10'}`}
            >
          ...
          {creation && !building && (
            <>
              <button onClick={undoBuild} disabled={!creation.versions || creation.versions.length < 2 || (creation.versionIndex ?? creation.versions.length - 1) === 0} title="Undo last change" className="flex min-h-9 flex-shrink-0 items-center gap-1.5 rounded-lg border border-violet-400/20 bg-violet-500/10 px-3 text-xs font-medium text-zinc-300 transition-all hover:bg-violet-500/10 disabled:opacity-40"><Undo2 size={14}/> <span className="hidden sm:inline">Undo</span></button>
              <button onClick={redoBuild} disabled={!creation.versions || (creation.versionIndex ?? creation.versions.length - 1) >= creation.versions.length - 1} title="Redo" className="flex min-h-9 flex-shrink-0 items-center gap-1.5 rounded-lg border border-violet-400/20 bg-violet-500/10 px-3 text-xs font-medium text-zinc-300 transition-all hover:bg-violet-500/10 disabled:opacity-40"><Redo2 size={14}/> <span className="hidden sm:inline">Redo</span></button>
            </>
          )}
          <button onClick={() => void runBuild()} disabled={building} className="flex min-h-9 flex-shrink-0 items-center gap-1.5 rounded-lg bg-violet-500/10 px-4 text-xs font-semibold text-white transition-all hover:bg-blue-500/10 disabled:opacity-50">
      ...
      {notice && <div className="border-b border-violet-400/20 bg-violet-500/10 px-5 py-3 text-center text-sm text-white/80">{notice}</div>}

      <div className="grid grid-cols-2 border-b border-violet-400/20 bg-violet-500/10 p-2 lg:hidden">
        <button onClick={() => setMobileView('chat')} className={`flex min-h-11 items-center justify-center gap-2 rounded-lg text-sm ${mobileView === 'chat' ? 'bg-violet-500/10 text-white' : 'text-white/55'}`}><MessageCircle size={16}/> Alpha Log</button>
        <button onClick={() => setMobileView('preview')} className={`flex min-h-11 items-center justify-center gap-2 rounded-lg text-sm ${mobileView === 'preview' ? 'bg-violet-500/10 text-white' : 'text-white/55'}`}><Eye size={16}/> Workspace</button>
      </div>
      ...
      <div className="grid min-h-0 flex-1 min-w-0 grid-cols-1 md:grid-cols-[minmax(0,420px)_4px_minmax(0,1fr)] lg:grid-cols-[minmax(0,420px)_4px_minmax(0,1fr)]" style={{ gridTemplateColumns: isMobile ? undefined : `minmax(0, ${leftWidth}px) 4px minmax(0, 1fr)` }}>
        <section className={`${mobileView === 'chat' ? 'flex' : 'hidden'} min-h-0 min-w-0 flex-1 flex-col border-b border-violet-400/20 bg-background md:flex md:border-b-0`}>
            ...
            <AssistantMessage>
              ...
              <div className="mt-3 flex flex-wrap gap-2">
                {['Make it blue', 'Add dark mode toggle', 'Add a contact form', 'Add charts and analytics'].map(s => (
                  <button key={s} onClick={() => void send(s)} className="rounded-full border border-violet-400/20 bg-violet-500/10 px-3 py-1.5 text-xs text-zinc-300 transition-all hover:border-indigo-400/40 hover:bg-violet-500/10 hover:text-white">{s}</button>
                ))}
              </div>
            </AssistantMessage>
            {chats.map(message => (
              <div key={message.id} className={`flex gap-3 ${message.role === 'user' ? 'flex-row-reverse' : 'flex-row'}`}>
                ...
                <div className={`max-w-[85%] rounded-2xl px-5 py-3 text-sm leading-7 ${message.role === 'user' ? 'bg-violet-500/10 text-white' : 'border border-violet-400/20 bg-violet-500/10 text-white/90 '}`}>
                  {message.content}
                </div>
              </div>
            ))}
          ...
          <div className="border-t border-violet-400/20 bg-violet-500/10 p-4">
            <div className="rounded-xl border border-violet-400/20 bg-violet-500/10 p-2 focus-within:border-violet-400/20">
              ...
              <div className="flex justify-end"><button onClick={() => void send()} disabled={pending || !input.trim()} className="grid size-11 place-items-center rounded-lg bg-violet-500/10 text-white transition-all hover:bg-blue-500/10 disabled:opacity-30" aria-label="Send"><ArrowUp size={18}/></button></div>
            </div>
          </div>
          <div className="border-t border-violet-400/20 bg-violet-500/10"><button onClick={() => {}} className="flex min-h-12 w-full items-center justify-between px-5 text-sm font-medium"><span>Build progress</span><span className="text-xs text-white/55">{activities.length} updates</span></button><ActivityFeedPanel activities={activities} building={building} /> {isMentorMission(mission.goal) && <MentorPanel mission={mission} />}</div>
        </section>

        <div
          onMouseDown={() => { resizingRef.current = true; document.body.style.userSelect = 'none'; document.body.style.cursor = 'col-resize' }}
          className="hidden cursor-col-resize bg-violet-500/10 hover:bg-violet-500/10 active:bg-violet-500/10 md:block"
          style={{ width: 4 }}
        ...
        <section className={`${mobileView === 'preview' ? 'flex' : 'hidden'} min-h-0 min-w-0 flex-1 flex-col bg-background p-3 sm:p-4 md:p-6 md:flex`}>
          {tab === 'preview' && (
            <div className="flex h-full min-h-0 flex-col rounded-2xl border border-violet-400/20 bg-violet-500/10 p-3 ">
              <div className="flex min-h-11 flex-wrap items-center gap-2 rounded-t-xl border-b border-violet-400/20 px-4 pb-3">
                ...
                <div className="min-w-0 flex-1 truncate rounded-lg border border-violet-400/20 bg-violet-500/10 px-3 py-1.5 text-center text-xs text-white/60">{previewUrl ? previewUrl.replace(/^.*\/preview\//, 'preview/') : (appLike ? 'phone.alphatekx.app' : 'preview.alphatekx.app')}</div>
                <div className="flex items-center gap-1 rounded-lg border border-violet-400/20 bg-violet-500/10 p-0.5">
                  <button onClick={() => setPreviewMode('desktop')} title="Desktop" className={`grid size-7 place-items-center rounded-md ${previewMode === 'desktop' ? 'bg-violet-500/10 text-white' : 'text-white/60 hover:text-white'}`}><Monitor size={14} /></button>
                  <button onClick={() => setPreviewMode('tablet')} title="Tablet" className={`grid size-7 place-items-center rounded-md ${previewMode === 'tablet' ? 'bg-violet-500/10 text-white' : 'text-white/60 hover:text-white'}`}><Tablet size={14} /></button>
                  <button onClick={() => setPreviewMode('phone')} title="Phone" className={`grid size-7 place-items-center rounded-md ${previewMode === 'phone' ? 'bg-violet-500/10 text-white' : 'text-white/60 hover:text-white'}`}><Smartphone size={14} /></button>
                </div>
                <div className="flex items-center gap-1 rounded-lg border border-violet-400/20 bg-violet-500/10 p-0.5">
                ...
                {creation && (
                  <div className="relative">
                    <button onClick={() => setPagesOpen(v => !v)} className="flex min-h-7 items-center gap-1.5 rounded-lg border border-violet-400/20 bg-violet-500/10 px-2 text-xs text-white/70 hover:bg-violet-500/10">
                    ...
                    {pagesOpen && (
                      <div className="absolute right-0 top-full z-50 mt-1 max-h-64 w-56 overflow-y-auto rounded-xl border border-violet-400/20 bg-[#151515] p-1 shadow-xl">
                        {filePages.length ? filePages.map(p => (
                          <button key={p.path} onClick={() => { setPagesOpen(false); previewIframeRef.current?.contentWindow?.postMessage({ type: 'alpha-navigate', view: p.name.toLowerCase() }, '*') }} className="w-full rounded-lg px-3 py-2 text-left text-xs text-zinc-300 hover:bg-violet-500/10">{p.name}</button>
                        )) : (
              ...
              <div className={`relative min-h-0 flex-1 rounded-b-xl border border-violet-400/20 bg-[#0A0F1E]/55 ${previewMode === 'phone' || previewMode === 'tablet' ? 'flex justify-center' : ''}`}>
                {previewError && (
                  <div className="absolute inset-0 z-20 flex flex-col items-center justify-center gap-3 rounded-b-xl bg-[#0A0F1E]/90 p-6 text-center">
                    ...
                    <button onClick={refreshPreview} className="rounded-lg bg-violet-500/10 px-4 py-2 text-xs font-semibold text-white hover:bg-blue-500/10">Try again</button>
                  </div>
                )}
                {previewLoading && (
          ...
          {tab === 'store' && (
            <div className="flex h-full min-h-0 flex-col rounded-2xl border border-violet-400/20 bg-violet-500/10 p-4 ">
              ...
              <div className="mt-3 min-h-0 flex-1 space-y-2 overflow-y-auto pr-1">
                {storeLoading ? (
                  ...
                  <button key={item.id} onClick={() => setInput(prev => prev ? `${prev}\n${item.content}` : item.content)} className="w-full rounded-xl border border-violet-400/20 bg-violet-500/10 p-3 text-left transition-colors hover:bg-violet-500/10">
              ...
              <div className="flex items-center justify-between">
                ...
                {creation && (
                  <div className="flex items-center gap-2">
                    <button onClick={() => navigator.clipboard.writeText(creation.code)} className="flex min-h-9 items-center gap-2 rounded-lg border border-violet-400/20 bg-violet-500/10 px-3 text-xs transition-all hover:bg-violet-500/10"><Copy size={13} /> Copy entry</button>
                    <button onClick={() => void exportCreationZip(creation)} className="flex min-h-9 items-center gap-2 rounded-lg border border-violet-400/20 bg-violet-500/10 px-3 text-xs transition-all hover:bg-violet-500/10"><Download size={13} /> Project ZIP</button>
                  </div>
                )}
              </div>
              <div className="mt-3 min-h-0 flex-1 overflow-hidden rounded-xl border border-violet-400/20 bg-[#0A0F1E] flex">
                <div className="w-52 border-r border-violet-400/20 overflow-y-auto p-2">
                  {fileList.map((file) => {
                    ...
                    return (
                      <button
                        ...
                        className={`flex w-full items-center gap-2 rounded-lg px-2 py-1.5 text-left text-xs ${selectedFile === file.path ? 'bg-violet-500/10 text-white' : 'text-zinc-400 hover:bg-violet-500/10 hover:text-zinc-200'}`}
                      >
          ...
          {tab === 'ai' && (
            <div className="flex h-full min-h-0 flex-col rounded-2xl border border-violet-400/20 bg-violet-500/10 p-5 ">
              ...
              {suggestions.length > 0 && (
                <div className="mt-4 flex flex-wrap gap-2">
                  {suggestions.map(s => (
                    <button key={s.title} onClick={() => { setAiInput(s.description); void planAgent() }} className="rounded-full border border-violet-400/20 bg-violet-500/10 px-3 py-1.5 text-xs text-zinc-300 transition-all hover:border-indigo-400/40 hover:bg-violet-500/10 hover:text-white">
                      {s.title}
                    </button>
                  ))}
                </div>
              ...
              {aiDraft && (
                <div className="mt-6 rounded-xl border border-violet-400/20 bg-violet-500/10 p-4">
                  ...
                  <div className="mt-3 flex flex-wrap gap-2 text-xs text-white/55">
                    <span className="rounded-lg bg-violet-500/10 px-2 py-1">Trigger: {aiDraft.trigger.type === 'schedule' ? (aiDraft.trigger.cron || 'Daily') : aiDraft.trigger.type}</span>
                    <span className="rounded-lg bg-violet-500/10 px-2 py-1">Cost: {aiDraft.creditsNeeded || aiDraft.creditsPerRun || 1} credit(s)</span>
                  </div>
                  ...
                  <div className="mt-4 flex gap-2">
                    <button onClick={() => void approveAndSaveAgent()} className="flex min-h-10 flex-1 items-center justify-center gap-2 rounded-lg bg-violet-500/10 px-4 text-sm font-semibold text-white transition-all hover:bg-blue-500/10">
                      <Plus size={16}/> Create & run
                    </button>
                    <button onClick={() => setAiDraft(null)} className="flex min-h-10 items-center justify-center gap-2 rounded-lg border border-violet-400/20 px-4 text-sm text-white transition-all hover:bg-violet-500/10">
                      Cancel
                    </button>
                  </div>
                </div>
            ...
            <div className="flex h-full min-h-0 flex-col rounded-2xl border border-violet-400/20 bg-violet-500/10 p-5 ">
              ...
              {creation ? (
                <div className="mt-6 rounded-xl border border-violet-400/20 bg-violet-500/10 p-4">
                  ...
                  <div className="mt-2 flex flex-col gap-2 sm:flex-row">
                    <div className={`flex min-h-11 min-w-0 flex-1 items-center rounded-lg border bg-violet-500/10 px-3 text-sm ${availability?.available === false ? 'border-red-500/50' : availability?.available === true ? 'border-emerald-500/50' : 'border-violet-400/20'}`}>
                        ...
                        {availability.suggestions && availability.suggestions.length > 0 && (
                          <span className="ml-2">
                            Try:{availability.suggestions.map(s => (
                              <button key={s} onClick={() => setDeploySlug(s)} className="ml-2 rounded bg-violet-500/10 px-1.5 py-0.5 text-emerald-300 hover:bg-violet-500/10">{s}</button>
                            ))}
                          </span>
                        )}
                      </span>
                  ...
                  {deploySlug && (
                    <div className="mt-4 rounded-lg border border-violet-400/20 bg-violet-500/10 p-3 text-xs text-white/70">
                ...
                <div className="mt-6 rounded-xl border border-dashed border-violet-400/20 p-8 text-center text-sm text-white/55">Build the mission first to unlock deploy.</div>
              )}
              ...
              <div className="mt-6 rounded-xl border border-violet-400/20 bg-violet-500/10 p-4">
                <div className="flex items-center justify-between">
                  ...
                  <button onClick={() => { setPasteMode(v => !v); setPasteResult(null); setNotice('') }} className="rounded-lg border border-violet-400/20 px-3 py-1.5 text-xs text-white transition-all hover:bg-violet-500/10">
                    {pasteMode ? 'Hide' : 'Paste code'}
                  </button>
                </div>
                {pasteMode && (
                  <div className="mt-4 space-y-3">
                    <input value={paste.title} onChange={e => setPaste(p => ({ ...p, title: e.target.value }))} className="w-full rounded-lg border border-violet-400/20 bg-violet-500/10 px-3 py-2 text-sm text-zinc-100 outline-none placeholder:text-slate-400" placeholder="App name" />
                    <div className={`flex min-h-11 min-w-0 items-center rounded-lg border bg-violet-500/10 px-3 text-sm ${pasteAvailability?.available === false ? 'border-red-500/50' : pasteAvailability?.available === true ? 'border-emerald-500/50' : 'border-violet-400/20'}`}>
                      <span className="hidden text-white/45 sm:inline">https://</span>
                    ...
                    <textarea value={paste.html} onChange={e => setPaste(p => ({ ...p, html: e.target.value }))} className="h-48 w-full rounded-lg border border-violet-400/20 bg-violet-500/10 p-3 text-xs font-mono text-zinc-100 outline-none placeholder:text-slate-400" placeholder="<!DOCTYPE html>..." />
                ...
                {pasteResult && (
                  <div className="mt-4 rounded-lg border border-emerald-500/20 bg-emerald-500/10 p-4">
                    ...
                    <button onClick={() => navigator.clipboard.writeText(pasteResult.url || pasteResult.pathUrl || pasteResult.subdomainUrl)} className="mt-2 flex min-h-9 items-center gap-2 rounded-lg bg-violet-500/10 px-3 text-xs font-medium text-white transition-all hover:bg-blue-500/10"><Copy size={13} /> Copy URL</button>
                  </div>
                )}
              </div>
              ...
              {deployResult && (
                <div className="mt-4 rounded-xl border border-emerald-500/20 bg-emerald-500/10 p-4">
                  ...
                  <div className="mt-3 flex flex-wrap gap-2">
                    <button onClick={() => navigator.clipboard.writeText(deployResult.url)} className="flex min-h-9 items-center gap-2 rounded-lg bg-violet-500/10 px-3 text-xs font-medium text-white transition-all hover:bg-blue-500/10"><Copy size={13} /> Copy URL</button>
                    <Link to={`/marketplace/new?previewUrl=${encodeURIComponent(deployResult.url)}&title=${encodeURIComponent(creation?.title || 'My AlphaTekX creation')}`} className="flex min-h-9 items-center gap-2 rounded-lg border border-violet-400/20 bg-violet-500/10 px-3 text-xs font-medium text-white transition-all hover:bg-violet-500/10"><ShoppingBag size={13} /> List for sale</Link>
                  </div>
                </div>
              )}
            </div>
...
function AssistantMessage({ children }: { children: ReactNode }) {
  return (
    <div className="flex gap-3">
      <div className="grid h-8 w-8 flex-shrink-0 place-items-center rounded-full bg-gradient-to-br from-indigo-500 to-pink-500 text-white"><Bot size={14} /></div>
      <div className="max-w-[85%] rounded-2xl border border-violet-400/20 bg-violet-500/10 px-5 py-3 text-sm leading-7 text-white/90 ">
        {children}
      </div>
    </div>
  )
  ...
  return (
    <div className="grid h-full min-h-0 place-items-center p-8 text-center">
      <div>
        ...
        {!building && onBuild && (
          <button onClick={onBuild} className="mt-5 rounded-xl bg-violet-500/10 px-5 py-2.5 text-sm font-semibold text-white transition-all hover:bg-blue-500/10">Build</button>
        )}
      </div>
    </div>
  )
...
function PlanPanel({ mission, building, onBuild }: { mission: Mission; building: boolean; onBuild: () => void }) {
  ...
  return (
    <div className="grid h-full min-h-0 place-items-center p-6">
      <div className="w-full max-w-lg rounded-2xl border border-violet-400/20 bg-violet-500/10 p-6 ">
        <h2 className="text-lg font-semibold">{plan.title}</h2>
        <p className="mt-1 text-sm text-white/55">{plan.description}</p>
        <div className="mt-4 space-y-2">
          {plan.modules.map((m, i) => (
            <div key={m.id} className="flex items-start gap-3 rounded-xl border border-violet-400/20 bg-violet-500/10 p-3">
              <span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-violet-500/10 text-xs font-bold">{i + 1}</span>
              <div>
        ...
        <div className="mt-5">
          <button onClick={onBuild} disabled={building} className="flex w-full items-center justify-center gap-2 rounded-xl bg-violet-500/10 py-2.5 text-sm font-semibold text-white transition-all hover:bg-blue-500/10 disabled:opacity-50">
