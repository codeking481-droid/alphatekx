    ...
    <div className="min-h-screen p-5 pb-28 md:p-8">
      <div className="mx-auto max-w-6xl">
        ...
        <div className="mt-5 grid grid-cols-2 gap-3 sm:grid-cols-4 md:grid-cols-6">
          {TYPES.map(t => (
            <button key={t} onClick={() => setTypeFilter(t)} className={`flex min-h-11 items-center justify-center gap-2 rounded-xl text-xs font-medium ${typeFilter === t ? 'btn-alpha text-white' : 'border border-violet-400/20 liquid-glass text-white/70'}`}>
              {t !== 'All' && iconForType(t as StoreItem['type'])} {t === 'All' ? 'All' : t} ({counts[t] || 0})
            </button>
          ))}
        </div>

        <div className="mt-5 flex flex-col gap-3 rounded-2xl border border-violet-400/20 liquid-glass p-4 md:flex-row md:items-end">
          <label className="flex min-h-12 flex-1 items-center gap-3 rounded-xl border border-violet-400/20 bg-background px-4">
          ...
          <input value={newItem.title} onChange={e => setNewItem(n => ({ ...n, title: e.target.value }))} className="min-h-12 flex-[2] rounded-xl border border-violet-400/20 bg-background px-4 text-sm outline-none" placeholder="Quick dump title" />
          <select value={newItem.type} onChange={e => setNewItem(n => ({ ...n, type: e.target.value as StoreItem['type'] }))} className="min-h-12 rounded-xl border border-violet-400/20 bg-background px-3 text-sm outline-none">
          ...
          <button onClick={() => fileInputRef.current?.click()} disabled={uploading} className="flex min-h-12 items-center gap-2 rounded-xl border border-violet-400/20 bg-violet-500/10 px-4 text-sm font-semibold text-white transition-transform hover:scale-[1.02] disabled:opacity-50">
            {uploading ? <LoaderCircle className="animate-spin" size={14}/> : <Upload size={14}/>} Upload
          </button>
          <button onClick={() => void quickAdd()} disabled={saving || !newItem.title.trim()} className="flex min-h-12 items-center gap-2 rounded-xl bg-violet-500/10 px-4 text-sm font-semibold text-white transition-transform hover:scale-[1.02] disabled:opacity-50">
            {saving ? <LoaderCircle className="animate-spin" size={14}/> : <Plus size={14}/>} Save
          </button>
        </div>

        {editing && (
          <div className="mt-5 rounded-2xl border border-violet-400/20 liquid-glass p-4">
            ...
            <div className="space-y-3">
              <input value={editing.title} onChange={e => setEditing({ ...editing, title: e.target.value })} className="min-h-11 w-full rounded-xl border border-violet-400/20 bg-background px-4 text-sm outline-none" />
              {editing.type === 'image' && <img src={editing.content} alt={editing.title} className="max-h-40 w-full rounded-lg object-cover" />}
              <textarea value={editing.content} onChange={e => setEditing({ ...editing, content: e.target.value })} rows={4} className="w-full rounded-xl border border-violet-400/20 bg-background px-4 py-3 text-sm outline-none" />
              <input value={editing.tags.join(', ')} onChange={e => setEditing({ ...editing, tags: e.target.value.split(',').map(t => t.trim()).filter(Boolean) })} className="min-h-11 w-full rounded-xl border border-violet-400/20 bg-background px-4 text-sm outline-none" placeholder="comma, separated, tags" />
              <button onClick={() => void save(editing)} disabled={saving} className="min-h-11 rounded-xl bg-violet-500/10 px-5 text-sm font-semibold text-white disabled:opacity-50">Save changes</button>
            </div>
          </div>
        )}

        {newItem.title && (
          <div className="mt-3 rounded-xl border border-violet-400/20 bg-violet-500/10 p-3 text-sm text-white/70">
        ...
        <div className="mt-6 flex items-center justify-between">
          <div className="flex items-center gap-2">
            <button onClick={() => setView('grid')} className={`rounded-lg p-2 ${view === 'grid' ? 'bg-violet-500/10 text-white' : 'text-white/50'}`}><Grid3X3 size={16}/></button>
            <button onClick={() => setView('list')} className={`rounded-lg p-2 ${view === 'list' ? 'bg-violet-500/10 text-white' : 'text-white/50'}`}><ListIcon size={16}/></button>
          </div>
          <select value={sort} onChange={e => setSort(e.target.value)} className="min-h-9 rounded-lg border border-violet-400/20 bg-background px-3 text-xs outline-none">
        ...
        {loading ? (
          <div className="mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-3">{[1,2,3,4,5,6].map(i => <div key={i} className="h-40 animate-pulse rounded-2xl bg-violet-500/10"/>)}</div>
        ) : items.length ? (
          <div className={`mt-6 grid gap-4 ${view === 'grid' ? 'md:grid-cols-2 xl:grid-cols-3' : 'grid-cols-1'}`}>
            {items.map(item => (
              <article key={item.id} className="flex flex-col rounded-2xl border border-violet-400/20 liquid-glass p-4 transition-all hover:border-indigo-400/30 hover:shadow-lg">
                ...
                {item.type === 'file' && <a href={item.content} download className="mt-3 inline-flex items-center gap-2 rounded-lg border border-violet-400/20 bg-violet-500/10 px-3 py-1.5 text-xs text-white/70 hover:bg-violet-500/10"><FileText size={12}/> Download file</a>}
                {item.type !== 'image' && item.type !== 'file' && <p className="mt-3 line-clamp-3 whitespace-pre-line text-xs text-white/55">{item.content}</p>}
                {item.tags.length > 0 && <div className="mt-3 flex flex-wrap gap-1.5">{item.tags.map(t => <span key={t} className="rounded-full bg-violet-500/10 px-2 py-0.5 text-[10px] text-white/70">{t}</span>)}</div>}
                <div className="mt-auto flex items-center gap-2 pt-4">
                  <button onClick={() => void copy(item.content)} className="flex min-h-8 items-center gap-1 rounded-lg border border-violet-400/20 px-2.5 text-xs transition-colors hover:bg-violet-500/10"><Copy size={12}/> Copy</button>
          ...
          <div className="mt-12 grid min-h-64 place-items-center rounded-2xl border border-dashed border-violet-400/20 liquid-glass text-center">
            <div>
