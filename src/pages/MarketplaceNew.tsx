  ...
  return (
    <div className="min-h-screen p-5 pb-28 md:p-8">
      <div className="mx-auto max-w-2xl">
        ...
        <div className="mt-6 space-y-4 rounded-2xl border border-violet-400/20 liquid-glass p-5 md:p-7">
          <label className="block">
            <span className="flex items-center gap-2 text-sm font-medium"><Type size={14}/> Title</span>
            <input value={product.title} onChange={e => setProduct(p => ({ ...p, title: e.target.value }))} className="mt-2 min-h-12 w-full rounded-xl border border-violet-400/20 bg-background px-4 text-sm outline-none focus:border-indigo-500" placeholder="e.g. SaaS Dashboard Kit" />
          </label>

          <label className="block">
            <span className="flex items-center gap-2 text-sm font-medium"><DollarSign size={14}/> Price (USD)</span>
            <input type="number" min={1} value={product.priceUSD} onChange={e => setProduct(p => ({ ...p, priceUSD: Number(e.target.value) }))} className="mt-2 min-h-12 w-full rounded-xl border border-violet-400/20 bg-background px-4 text-sm outline-none focus:border-indigo-500" placeholder="10" />
          </label>

          <label className="block">
            <span className="flex items-center gap-2 text-sm font-medium"><Tag size={14}/> Category</span>
            <select value={product.category} onChange={e => setProduct(p => ({ ...p, category: e.target.value }))} className="mt-2 min-h-12 w-full rounded-xl border border-violet-400/20 bg-background px-4 text-sm outline-none focus:border-indigo-500">
          ...
          <label className="block">
            ...
            <textarea value={product.description} onChange={e => setProduct(p => ({ ...p, description: e.target.value }))} rows={5} className="mt-2 w-full rounded-xl border border-violet-400/20 bg-background px-4 py-3 text-sm outline-none focus:border-indigo-500" placeholder="What does it do? Who is it for?" />
          </label>

          <label className="block">
            <span className="flex items-center gap-2 text-sm font-medium"><ImageIcon size={14}/> Thumbnail URL (optional)</span>
            <input value={product.thumbnail} onChange={e => setProduct(p => ({ ...p, thumbnail: e.target.value }))} className="mt-2 min-h-12 w-full rounded-xl border border-violet-400/20 bg-background px-4 text-sm outline-none focus:border-indigo-500" placeholder="https://..." />
          </label>

          <label className="block">
            <span className="text-sm font-medium">Live preview URL</span>
            <input value={product.previewUrl} onChange={e => setProduct(p => ({ ...p, previewUrl: e.target.value }))} className="mt-2 min-h-12 w-full rounded-xl border border-violet-400/20 bg-background px-4 text-sm outline-none focus:border-indigo-500" placeholder="https://alphatekx.name.ng/app/your-app" />
          </label>

          <label className="block">
            <span className="text-sm font-medium">Demo URL (public preview)</span>
            <input value={product.demoUrl} onChange={e => setProduct(p => ({ ...p, demoUrl: e.target.value }))} className="mt-2 min-h-12 w-full rounded-xl border border-violet-400/20 bg-background px-4 text-sm outline-none focus:border-indigo-500" placeholder="https://..." />
          </label>
