...
export default function Admin() {
  ...
  return <div className="mx-auto max-w-6xl px-5 py-20"><div className="flex flex-wrap items-center justify-between gap-4"><div><div className="flex items-center gap-2"><ShieldCheck size={20}/><h1 className="text-xl font-semibold">Admin</h1></div><p className="mt-2 text-sm text-white/55">Live AlphaTekX account activity. Refreshes every 15 seconds.</p></div><button onClick={()=>void load()} className="flex min-h-11 items-center gap-2 rounded-lg border border-white/[.15] liquid-glass px-4 text-sm"><RefreshCw size={16}/>Refresh</button></div>{error&&<p className="mt-5 rounded-lg border border-[#E56B2D]/40 bg-[#1E1A18] p-3 text-sm text-white">{error}</p>}{loading?<div className="grid min-h-64 place-items-center"><LoaderCircle className="animate-spin"/></div>:stats&&<><div className="mt-7 grid gap-4 sm:grid-cols-3"><Stat icon={Users} label="Total signups" value={stats.total}/><Stat icon={Activity} label="Active now" value={stats.active}/><Stat icon={UserPlus} label="Signed up today" value={stats.today}/></div><section className="mt-7 overflow-hidden rounded-xl border border-white/[.12] liquid-glass"><div className="border-b border-white/[.12] px-5 py-4"><h2 className="font-semibold">Users</h2></div><div className="overflow-x-auto"><table className="w-full min-w-[720px] text-left text-sm"><thead className="bg-black/30 text-xs text-white/55"><tr><th className="px-5 py-3">Email</th><th className="px-5 py-3">Plan</th><th className="px-5 py-3">Credits</th><th className="px-5 py-3">Joined</th><th className="px-5 py-3">Last active</th></tr></thead><tbody>{stats.users.map(item=><tr key={item.id} className="border-t border-white/10"><td className="px-5 py-4 font-medium">{item.email}</td><td className="px-5 py-4 capitalize">{item.plan}</td><td className="px-5 py-4">{item.email.toLowerCase()==='iamdan4live@gmail.com'?'Unlimited':item.credits}</td><td className="px-5 py-4 text-white/55">{new Date(item.created_at).toLocaleString()}</td><td className="px-5 py-4 text-white/55">{item.last_active_at?new Date(item.last_active_at).toLocaleString():'Not tracked yet'}</td></tr>)}</tbody></table></div></section></>}</div>
}
