    ...
    <main id="top" className="min-h-screen overflow-x-hidden bg-[#0A0A0F] text-white">
      ...
      <section className="relative isolate overflow-hidden px-4 pb-14 pt-12 sm:px-6 sm:pb-20 sm:pt-16 lg:px-8 lg:pt-24">
        ...
        <div className="mx-auto flex max-w-7xl flex-col items-center text-center">
          ...
          <motion.h1 initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="mt-6 max-w-5xl text-[2.35rem] font-black leading-[0.95] tracking-[-0.03em] text-white sm:text-5xl lg:text-7xl">
            YOUR SECOND YOU WORKS WHILE YOU SLEEP
          </motion.h1>
          <motion.p initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.55, delay: 0.05 }} className="mt-6 max-w-3xl text-sm leading-7 text-slate-400 sm:text-base sm:leading-8 lg:text-lg">
          ...
          <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.1 }} className="mt-8 flex w-full flex-col items-stretch justify-center gap-3 sm:w-auto sm:flex-row sm:items-center">
            <a href="#pricing" className="inline-flex w-full items-center justify-center gap-2 rounded-full bg-[#3B82F6] px-6 py-3.5 text-sm font-semibold text-white transition hover:bg-[#2563EB] sm:w-auto">
          ...
          <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, delay: 0.15 }} className="mt-12 w-full max-w-6xl overflow-hidden rounded-[2rem] border border-white/10 bg-[#0F1118]/90 shadow-[0_32px_120px_rgba(0,0,0,0.45)]">
      ...
      <section id="why-better" className="mx-auto max-w-7xl px-4 py-14 sm:px-6 sm:py-20 lg:px-8">
        <SectionTitle eyebrow="Why we are 10x better" title="Built for serious operators, not vanity posting." subtitle="A mature platform has to be safe, personal, and broad. This is the operating layer your team actually needs." />
        <div className="mt-10 grid gap-5 lg:grid-cols-3">
          {pillars.map((pillar) => (
            <GlassCard key={pillar.title} className="min-h-[220px]">
              <div className="flex h-11 w-11 items-center justify-center rounded-2xl bg-[#3B82F6]/12 text-[#3B82F6]">
      ...
      <section id="how-it-works" className="mx-auto max-w-7xl px-4 py-14 sm:px-6 sm:py-20 lg:px-8">
        <SectionTitle eyebrow="How it works" title="One Google Sheet. Three decisions. Full autopilot." subtitle="The workflow is simple enough to trust and powerful enough to scale." />
        <div className="mt-10 grid gap-6 lg:grid-cols-[0.95fr_1.05fr]">
          <div className="rounded-[2rem] border border-white/10 bg-white/[0.04] p-6">
            {steps.map((step, index) => (
              <div key={step.title} className="flex gap-4 border-b border-white/10 py-5 first:pt-0 last:border-b-0 last:pb-0">
                <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-[#3B82F6]/12 text-sm font-black text-[#3B82F6]">0{index + 1}</div>
                <div>
          ...
          <GlassCard className="flex flex-col justify-between">
            <div>
              <div className="flex items-center justify-between text-[11px] font-black uppercase tracking-[0.3em] text-slate-500">
                ...
                <span className="text-[#06FFA5]">Approved</span>
              </div>
              <div className="mt-6 overflow-hidden rounded-[1.25rem] border border-white/10">
                <div className="grid grid-cols-[1.2fr_0.8fr_0.8fr] bg-white/[0.06] px-4 py-3 text-xs font-black uppercase tracking-[0.28em] text-slate-400">
                  <span>Topic</span>
                  <span>Post</span>
                  <span>Approve</span>
                </div>
                {[
                  ["Founder thought", "Launch the month", "YES"],
                  ["Customer insight", "Reply with a case study", "YES"],
                  ["Build update", "Ship the weekly note", "NO"],
                ].map(([topic, post, action]) => (
                  <div key={topic} className="grid grid-cols-[1.2fr_0.8fr_0.8fr] border-t border-white/10 bg-[#090B12] px-4 py-3 text-sm text-slate-300">
                    <span>{topic}</span>
                    <span>{post}</span>
                    <span className={action === "YES" ? "font-semibold text-[#06FFA5]" : "font-semibold text-slate-500"}>{action}</span>
                  </div>
                ))}
              </div>
            </div>
            <div className="mt-6 rounded-[1.2rem] border border-[#06FFA5]/20 bg-[#06FFA5]/10 p-4 text-sm text-slate-300">
              Approved items ship. Rejected items stay in the queue. The system stays controlled.
            </div>
          </GlassCard>
        </div>
      </section>
      ...
      <section className="mx-auto max-w-7xl px-4 py-14 sm:px-6 sm:py-20 lg:px-8">
        <SectionTitle eyebrow="Platforms" title="Six matured platforms. One operating layer." subtitle="This is the real stack for builders who need leverage without noise." />
        <div className="mt-10 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          {platforms.map((platform) => (
            <div key={platform.name} className="rounded-[1.5rem] border border-white/10 bg-white/[0.04] p-5">
              <div className="flex items-center gap-3 text-[#3B82F6]">{platform.icon}<h3 className="text-lg font-semibold text-white">{platform.name}</h3></div>
            </div>
          ))}
        </div>
      </section>
      ...
      <section id="pricing" className="mx-auto max-w-7xl px-4 py-14 sm:px-6 sm:py-20 lg:px-8">
        <SectionTitle eyebrow="Pricing" title="Cheaper than Taplio. Six times more powerful." subtitle="The value is not in one channel. It is in one system that keeps the whole motion alive." />
        <div className="mt-10 grid gap-6 lg:grid-cols-[1.05fr_0.95fr]">
          <GlassCard className="border-[#3B82F6]/25 bg-gradient-to-br from-[#3B82F6]/12 to-[#8B5CF6]/10">
            <p className="text-[11px] font-black uppercase tracking-[0.32em] text-[#06FFA5]">Early Bird</p>
            ...
            <p className="mt-4 text-sm leading-7 text-slate-400">First 100 locked forever. Includes all six platforms, unlimited AI, voice training, Sheets OS, and Pollinations images.</p>
            <ul className="mt-6 space-y-3 text-sm text-slate-300">
              <li className="flex items-center gap-3"><CheckCircle2 className="h-4 w-4 text-[#06FFA5]" /> Six platforms, one workflow</li>
              <li className="flex items-center gap-3"><CheckCircle2 className="h-4 w-4 text-[#06FFA5]" /> Unlimited AI generation</li>
              <li className="flex items-center gap-3"><CheckCircle2 className="h-4 w-4 text-[#06FFA5]" /> Voice training from your records</li>
            </ul>
            <a href="#top" className="mt-8 inline-flex items-center gap-2 rounded-full bg-[#3B82F6] px-5 py-3 text-sm font-semibold text-white transition hover:bg-[#2563EB]">
              Access AlphaTekX OS Now
              <ArrowRight className="h-4 w-4" />
            </a>
          </GlassCard>
          <div className="rounded-[2rem] border border-white/10 bg-white/[0.04] p-6">
            <div className="flex items-center gap-2 text-sm font-semibold text-slate-300"><ChevronRight className="h-4 w-4 text-[#3B82F6]" /> Taplio $39 = one platform, zero AI credits</div>
            <div className="mt-4 flex items-center gap-2 text-sm font-semibold text-slate-300"><ChevronRight className="h-4 w-4 text-[#3B82F6]" /> AlphaTekX $19 = six platforms, unlimited output</div>
