import{g as u,p as o}from"./apiClient-QTXSi6tu.js";const g=/^[a-z0-9](?:[a-z0-9-]{1,28}[a-z0-9])$/;function d(t){var i;let e=String(t||"");const n=[],a=/\bimport\s+(?:type\s+)?([\s\S]*?)\s+from\s+(['"])([^'"]+)\2\s*;?/g;for(const r of e.matchAll(a)){if(r[3]!=="lucide-react")continue;const s=((i=r[1].match(/\{([\s\S]*?)\}/))==null?void 0:i[1])||"";for(const p of s.split(",")){const c=p.trim().split(/\s+as\s+/i),l=c[1]||c[0];/^[A-Za-z_$][\w$]*$/.test(l)&&n.push(l)}}return e=e.replace(a,`
